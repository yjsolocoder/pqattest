...
:func:`recommend_merkle_verify_mode_deployment` applies those same five
preferences to the fixed-leaf-position
:func:`merkle_verify_mode_frontier` frontier and returns one
:class:`MerkleModeCost`.
...
def _validate_weight_scenarios(scenarios: Any) -> tuple[tuple[int, ...], ...]:
    ...
    Shared by the fixed-position and cardinality multi-scenario recommenders;
    the accepted shapes, types and exception wording are the same for both.
    """
...
def recommend_merkle_verify_mode_deployment(
    ...
    returned fixed-leaf-position mode/verify-cost plans by a business
    preference instead of returning them all. The five ranked costs of each
    survivor are aggregate transport bytes
    (:attr:`MerkleTransportWorkloadProfile.total`), any single group's
    transport peak (the maximum of
