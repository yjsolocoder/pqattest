...
- `sign_merkle_auth_state_batch(data, messages, *, key, min_generation=None, claim) -> (signatures, envelope, generation)` — 上述转换的**无隐藏状态批量版**：认证恢复流程完全相同（v2 验 HMAC、固定 `"merkle"`、代次下限、恢复 v1 载荷），随后按 `MerkleSigner.sign_batch` 的规则从恢复签名器的当前最小叶起对**非空元组** `messages` 的每个成员（沿用 `bytes`/`bytearray`/`str` 规则，`str` 按 UTF-8 编码）在连续叶子上各签一条，再把推进后（`next_index` 一次加上批次长度）的 v1 检查点以 `scheme="merkle"`、原 `key` 与 **g+1** 代次封装。返回依次为签名元组（与同状态下 `MerkleSigner.sign_batch(messages)` 逐值相同，索引严格递增）、封装字节（与对批签后检查点直接调用 `auth_state_wrap` 逐字节相同）与 `g+1`。不修改任何对象、不保留库内状态、不取随机数、不新增线格式，同一封装重复调用结果逐字节相同。`messages` 必须为**非空 tuple**：非元组或成员类型非法抛 `TypeError`，空元组抛 `ValueError`；`data`/`key` 仅收 `bytes`/`bytearray` 且 key 非空，`min_generation` 为 `None` 或非布尔 uint64，`claim` 必须可调用，否则按既有规则抛 `TypeError`/`ValueError`。剩余叶子不足以容纳整批时抛 `KeyExhaustedError`（不消耗任何叶子）；`g` 为 `2**64-1` 时抛 `ValueError`。所有签名、候选检查点与封装都齐备后，才以唯一入参 `(("merkle", g), ("merkle", g+1))` 恰好调用一次 `claim`，仅返回值 `is True` 时成功（否则抛 `ValueError`），回调异常原样透传；任何先前失败都不调用 `claim`、不推进状态、无部分返回。旧接口与线格式保持不变
...
一批消息只推进一代时用无状态批量转换 `sign_merkle_auth_state_batch`：恢复同一封装后，消息按顺序占用从当前最小叶起的连续叶子，全部签名与 `g+1` 封装齐备后才进行同一次成对认领，整批要么一起成功要么什么都不发生：

```python
from pqattest import sign_merkle_auth_state_batch

signatures, next_blob, generation = sign_merkle_auth_state_batch(
    blob, (b"claim-0", b"claim-1", "claim-2"), key=key,
    min_generation=state["high_water"], claim=claim_transition,
)
assert len(signatures) == 3              # 连续叶子上的三条签名
for message, signature in zip((b"claim-0", b"claim-1", b"claim-2"), signatures):
    assert merkle_verify(message, signature, signer.public_key)
# 剩余叶子不足整批时抛 KeyExhaustedError，且不消耗任何叶子、不调用 claim
```
