...
- `merkle_deployment_frontier(capacity, budgets)` — 与 `recommend_merkle_deployment` 同一组普通部署指标，但**不排序取首项**，而是返回全部可行且非支配的部署，类型为 `tuple[MerkleStorageProfile, ...]`，不新增值类型。纯函数：不取随机数、不生成密钥、不改状态，且无默认参数。`capacity` 限 1 至 256 的非布尔整数；`budgets` 必须为四元组，按顺序分别为检查点字节、单签线长、独立证明线长及单签验签步数（`profile("merkle", ...).steps`）的上限，各项为 `None`（不限）或正的非布尔整数，且至少一项非空，所有预算均为含边界上限。枚举 `w=4/8` × `height=1..8` 且叶数覆盖 `capacity` 的全部候选，尺寸取 `merkle_storage_profile`、步数取 `profile`。支配判定：A 在检查点字节、签名线长、证明线长、单签步数四项上均不大于 B 且至少一项严格更小，则 A 支配 B；删除全部被支配项并按值去重，不因尺寸或速度偏好预先舍弃构成取舍的配置。结果按单签步数、签名线长、证明线长、检查点字节、叶数、`w`、`height` 稳定升序排列。`budgets` 非元组抛 `TypeError`，其余非法输入或无可行候选抛 `ValueError`
...
# 前沿：同一部署预算下全部可行且非支配的配置，调用方可同时查看存储、签名、
# 证明线长与验签步数之间的取舍，按步数、签名、证明、检查点、叶数、w、height 升序
