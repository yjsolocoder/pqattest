...
class SignWithCheckpointTest(unittest.TestCase):
    ...
    def test_signature_verifies(self):
        ...
        signature, _ = signer.sign_with_checkpoint("m")
        self.assertTrue(verify("m", signature, signer.public_key))
    ...
    def test_accepts_bytes_bytearray_and_str(self):
        for message in (b"m", bytearray(b"m"), "m"):
            with self.subTest(message=type(message).__name__):
                ...
                self.assertTrue(verify(b"m", signature, signer.public_key))
    ...
    def test_restored_checkpoint_is_exhausted_with_same_public_key(self):
        ...
        with self.assertRaises(KeyExhaustedError):
            restored.sign("other")
        # A second restore of the same bytes behaves identically.
        restored_again = OneTimeSigner.from_checkpoint(bytes(blob))
    ...
    def test_draws_no_randomness(self):
        ...
        with mock.patch.object(
            ...
            signature, blob = signer.sign_with_checkpoint("m")
        self.assertEqual(len(signature), signer.public_key.bits)
        self.assertEqual(blob[9], 1)
    ...
    def test_used_signer_raises_without_partial_result(self):
        ...
        # No state movement: a checkpoint now equals one taken before.
...
class SignWithCheckpointConcurrencyTest(unittest.TestCase):
    def test_linearises_with_sign_and_checkpoint(self):
        ...
        # Exactly one signature across both the atomic and plain paths.
        self.assertEqual(len(atomic) + len(plain), 1)
        self.assertTrue(signer.used)

        # Every snapshot is a whole state: same public key, used flag
        # consistent with the bytes, and the atomic blob (if any) restores
        # an exhausted signer whose key matches the live one.
