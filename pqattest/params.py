...
:func:`recommend_merkle_transport_deployment_weighted_scenarios` applies
the preference-drift-robust multi-scenario minimax-regret ranking to that
same frontier, evaluating a non-empty tuple of such five-weight scenarios
at once with the same exact :class:`fractions.Fraction` arithmetic.
...
def recommend_merkle_transport_deployment_weighted_scenarios(
    ...
    :func:`merkle_transport_deployment_frontier`'s returned frontier with
    the smallest worst-case regret — the maximum, over scenarios, of how
    much worse the chosen deployment scores than the best deployment for
    that scenario. The five weighted costs are the same ones
    ...
    the multi-proof wire bytes (:attr:`MerkleTransportDeploymentProfile.multi`),
    the carried multi-proof node count
    ...
    nodes, checkpoint bytes and per-signature chain steps. Every weight
    must be a non-boolean, non-negative integer and at least one weight of
    each scenario must be positive; repeated scenarios are counted
    separately, exactly like repeated leaf indices.
    ...
    candidate's score to give that scenario's regret; deployments are ranked
    first by the greatest regret over the scenarios (the minimax-regret
    choice), then by the sum of the regrets, then by the tuple of
    per-scenario scores, all ascending. Every ranking finishes with the same
    ascending tie-break as every other joint-deployment ranking — checkpoint
    bytes, leaf count, ``w`` and ``height`` — and the first survivor is
    returned as a :class:`MerkleTransportDeploymentProfile`.
