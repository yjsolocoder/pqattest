...
- `MerkleVerifyWorkloadProfile` — 冻结的多组验签哈希成本汇总值对象，四个字段按位置依次为 `w, height, costs, total`，类型依次为 `int, int, tuple[tuple[str,int,int,int,int], ...], int`；`costs` 与输入各组同序，每项五元组依次为模式名（`"batch"` 或 `"multiproof"`）、W-OTS 链步上界、叶哈希数、该模式的内部节点哈希数及本组三项哈希总数；`total` 为各组总数之和，重复组也分别计费；冻结、可位置构造、按值相等（可哈希）
- `merkle_verify_workload_profile(w, height, groups, modes)` — 纯函数：为**多个独立叶索引组**汇总批次/多证明**混合方案**的验签 SHA-256 工作量，返回 `MerkleVerifyWorkloadProfile`，不生成密钥、不取随机数、不修改状态，旧接口与格式不变。`w`、`height` 与 `groups` 中每组完全沿用 `merkle_verify_profile` 的单组规则（每组均为非空、严格递增、落在 `0 .. 2**height-1` 内的非布尔整数元组）；`groups` 必须为非空元组，`modes` 必须为与 `groups` 等长的元组且只含 `"batch"`/`"multiproof"`。每组复用 `merkle_verify_profile(w, height, group)`：模式 `"batch"` 的内部成本取其 `batch`，模式 `"multiproof"` 取其 `multi`；五元组为 `(mode, wots, leaf, internal, wots+leaf+internal)`，`total` 为各组第五项之和，同一组重复出现也逐组分别计费。`groups`/`modes`（含成员组本身）容器错型抛 `TypeError`；`groups` 为空或含空组、`modes` 长度不符、出现未知模式，以及非法索引、`w` 或 `height` 均抛 `ValueError`
...
# 多组混合方案：每组各选 batch 或 multiproof，逐组复用 merkle_verify_profile，
# costs 与 groups 同序（mode, W-OTS 链步, 叶哈希, 内部节点哈希, 本组合计），
# total 为各组合计之和；重复组也分别计费
merkle_verify_workload_profile(
    8, 3, ((0, 1), (0, 1, 6), (0, 1)), ("batch", "multiproof", "multiproof")
)
# MerkleVerifyWorkloadProfile(w=8, height=3, costs=(
#     ("batch", 2*34*255, 2, 2*3, 2*34*255 + 2 + 2*3),
#     ("multiproof", 3*34*255, 3, 5, 3*34*255 + 3 + 5),
#     ("multiproof", 2*34*255, 2, 3, 2*34*255 + 2 + 3)), total=...)
```
