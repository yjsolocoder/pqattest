...
class RecommendMerkleTransportWorkloadWeightedScenariosTest(unittest.TestCase):
    ...
    def test_repeated_scenarios_counted_separately(self):
        ...
        first = (1, 0, 0, 0)
        second = (0, 0, 0, 1)
        balanced = recommend_merkle_transport_workload_weighted_scenarios(
            16, _GROUPS, _BUDGETS, (first, second)
        )
        repeated = recommend_merkle_transport_workload_weighted_scenarios(
            16, _GROUPS, _BUDGETS, (first, second, second)
        )
        self.assertEqual((balanced.config.w, balanced.config.height), (8, 4))
        self.assertEqual((repeated.config.w, repeated.config.height), (4, 4))
