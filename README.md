...
- `MerkleSigner(*, height=4, w=4, token_bytes=secrets.token_bytes)` — 生成 `2**height` 把 W-OTS 密钥并建成 Merkle 树；`height` 为 1 至 8 的整数（非布尔），`w` 为 4 或 8。只读属性 `public_key`、`next_index`（下一可分配叶索引）与 `remaining`（恒等于 `public_key.leaf_count - next_index`，非负）；`sign(message)` 线程安全地按 0 起递增分配叶子，仅成功后消耗叶子（非法消息抛 `TypeError` 且不消耗），叶子用尽抛 `KeyExhaustedError`，并发调用不会分配到重复索引
- `MerkleSigner.advance_to(next_index)` — 供崩溃恢复/状态不确定时**主动作废**可能已暴露的叶子：把分配前沿只向前推进到 `next_index`，被跳过的叶子永远不会再被 `sign`/`sign_batch` 分配，**不提供任何公开回退入口**。返回推进前后的前沿 `(旧 next_index, 新 next_index)`；目标等于当前索引时返回相同二元组、不改状态、不取任何随机数。`next_index` 只接受非布尔整数（布尔或其他类型抛 `TypeError`），且必须落在闭区间 `[当前 next_index, public_key.leaf_count]`（倒退或超过叶总数抛 `ValueError`）。与 `sign`、`sign_batch`、`checkpoint` 及两个属性读共用同一把锁并线性化；推进只改下一索引、不改公私钥；推进到叶总数后 `remaining` 为 0，`sign` 与非空 `sign_batch` 抛 `KeyExhaustedError`（空 `sign_batch(())` 仍返回 `()`）
- `MerkleSigner.checkpoint()` — 把完整签名状态（含**全部私钥**）序列化为 `bytes`；与 `sign`、`sign_batch`、`advance_to` 及属性读共用同一把锁，并发快照只会落在某次签名或推进之前或之后，不会落在其中途。v1 格式不变：推进状态就保存在原有 2 字节 `next_index` 字段中，故旧/新检查点完全兼容
