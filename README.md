...
- `sign_multiproof_merkle_auth_state(data, messages, *, indices=None, key, min_generation=None, claim) -> (proof, envelope, generation)` — **无隐藏状态**的「认证恢复 + 指定叶批量签名 + 去重多证明 + 下一代封装」转换，是 `sign_merkle_auth_state_batch` 的多证明对应物，并允许指定叶索引：`data` 为 `"merkle"` 的 v2 封装（`bytes`/`bytearray`），先按既有 v2 规则用 `hmac.compare_digest` 验 HMAC、固定方案、应用代次下限并恢复原样 v1 载荷，再在一次临界区内对 `messages` 逐份签名并把恢复公钥与这些签名交给 `multiproof_encode` 压成一份去重认证路径的证明（沿用既有 v1 叶块与规范节点顺序），最后把推进后的 v1 检查点以 `scheme="merkle"`、原 `key` 与 **g+1** 代次封装。`indices=None`（默认）时与 `sign_merkle_auth_state_batch` 完全一致：自恢复签名器的**当前最小叶**起**连续**签名，`next_index` 推进批长；显式给出 `indices` 时它必须是与 `messages` **等长**的元组，成员为自当前叶至末叶（含）范围内的**非布尔整数**且**严格递增**，第 i 条消息在第 i 个索引的叶子上签名，索引之间（以及当前叶到首个索引之间）的**间隙叶全部作废**，签后 `next_index` 为末项加一。`messages` 必须是**非空元组**，成员沿用 `bytes`/`bytearray`/`str`（UTF-8）规则；`data`/`key` 仅收 `bytes`/`bytearray` 且 `key` 非空；`indices` 仅收 `None` 或非布尔整数元组；`min_generation` 为 `None` 或非布尔 uint64；`claim` 须可调用。返回 `(proof, envelope, generation)`：`proof` 为 `bytes`，与对同状态在所选叶上取得的签名调用 `multiproof_encode` **逐字节相同**，且 `multiproof_verify(messages, proof)` 返回 `True`；`envelope` 与对签后（`next_index` 为末索引加一）检查点直接调用 `auth_state_wrap` **逐字节相同**；`generation` 为 `g+1`。不修改任何对象、不保留库内状态、不取随机数、不新增线格式，同一输入重复调用逐字节相同。`indices=None` 时整批容量不足抛 `KeyExhaustedError`（沿用 `sign_merkle_auth_state_batch`）；`indices` 非元组成员含布尔/非整数抛 `TypeError`，长度不等、非严格递增、低于当前叶或越过末叶抛 `ValueError`；入参 `g` 必须小于 `2**64-1`（否则 `ValueError`）。**全部输出生成后**才以唯一入参 `(("merkle", g), ("merkle", g+1))` 恰好调用一次 `claim`，仅返回值 `is True` 时成功（否则抛 `ValueError`），回调异常原样透传；任何先前失败（含错型、空批、空 key、认证失败、代次低于下限、代次触顶、索引非法、容量不足、检查点非法、认领拒绝）都**不调用** `claim`、不推进且无部分返回
...
若只想在指定（可带间隙的）叶子上签名、并拿到一份可独立传输的去重证明，用 `sign_multiproof_merkle_auth_state`：`indices=None` 时等同批签的多证明版；显式给出与消息等长、严格递增的叶索引元组时，各消息在指定叶上签名，**间隙叶一并作废**，签后 `next_index` 为末索引加一，证明可用 `multiproof_verify` 直接验证：

```python
from pqattest import sign_multiproof_merkle_auth_state, multiproof_verify

# indices=None：自当前叶起连续签名（与批签同语义）
proof, next_blob, generation = sign_multiproof_merkle_auth_state(
    blob, (b"claim 0", b"claim 1", b"claim 2"), key=key,
    min_generation=state["high_water"], claim=claim_transition,
)
assert multiproof_verify((b"claim 0", b"claim 1", b"claim 2"), proof)

# 显式索引：在叶子 0、2、4 上签名，叶子 1、3 作废，签后 next_index == 5
proof, next_blob, generation = sign_multiproof_merkle_auth_state(
    blob, (b"claim 0", b"claim 2", b"claim 4"), indices=(0, 2, 4), key=key,
    min_generation=state["high_water"], claim=claim_transition,
)
assert generation == 8
assert multiproof_verify((b"claim 0", b"claim 2", b"claim 4"), proof)
assert not multiproof_verify((b"claim 0", b"claim 2", b"other"), proof)
```
