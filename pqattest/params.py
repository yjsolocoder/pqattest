...
:func:`explain_merkle_transport_deployment_weighted_scenarios` exports
the decision-cost breakdown behind the multi-scenario joint-deployment
ranking as a tuple of frozen
:class:`MerkleTransportDeploymentScenarioScore` rows — one per frontier
member, in frontier order, each carrying the candidate's deployment, its
five normalised costs, its per-scenario scores and regrets and a selected
flag — with the same exact :class:`fractions.Fraction` arithmetic.
...
per-scenario scores) with the same exact :class:`fractions.Fraction`
arithmetic, and returns one profile.
:func:`explain_merkle_transport_workload_weighted` exports the
decision-cost breakdown behind the single-weight workload ranking as a
tuple of frozen :class:`MerkleTransportWorkloadScore` rows — one per
...
as a tuple of frozen
:class:`MerkleTransportWorkloadScenarioScore` rows — one per frontier
member, in frontier order, each carrying the candidate's workload
plan, its four normalised costs, its per-scenario scores and regrets
and a selected flag — with the same exact :class:`fractions.Fraction`
arithmetic.
...
:class:`fractions.Fraction` arithmetic, and returns one profile.
:func:`explain_merkle_mode_weighted` exports the decision-cost breakdown
behind the single-weight mode ranking as a tuple of frozen
:class:`MerkleModeScore` rows — one per frontier member, in frontier
...
:func:`explain_merkle_mode_weighted_scenarios` exports the decision-cost
breakdown behind the multi-scenario mode ranking as a tuple of frozen
:class:`MerkleModeScenarioScore` rows — one per frontier member, in
frontier order, each carrying the candidate's workload plan, its five
...
:func:`explain_merkle_deployment_weighted_scenarios` exports the
decision-cost breakdown behind the multi-scenario ordinary-deployment
ranking as a tuple of frozen :class:`MerkleDeploymentScenarioScore`
rows — one per frontier member, in frontier order, each carrying the
candidate's config, its four normalised costs, its per-scenario scores
and regrets and a selected flag — with the same exact
:class:`fractions.Fraction` arithmetic.
...
:class:`MerkleModeCost`.
:func:`explain_merkle_verify_mode_deployment_weighted` exports the
decision-cost breakdown behind that same weighted fixed-position ranking
as a tuple of frozen :class:`MerkleVerifyModeDeploymentScore` rows — one
per frontier member, in frontier order, each carrying the candidate's
plan/cost pairing, its five normalised costs, its final score and a
selected flag — with the same exact :class:`fractions.Fraction` arithmetic.
...
@dataclass(frozen=True)
class MerkleDeploymentScenarioScore:
    """Frozen multi-scenario breakdown for one ordinary Merkle deployment.

    One row of :func:`explain_merkle_deployment_weighted_scenarios`'s
    ...
    - ``scores``: the per-scenario weighted scores, in the same order as
      the input scenarios — each the four normalised costs times that
    ...
    - ``selected``: ``True`` on exactly the one row
      :func:`recommend_merkle_deployment_weighted_scenarios` would pick.
...
def explain_merkle_deployment_weighted_scenarios(
    capacity: Any,
    budgets: Any,
    scenarios: Any,
) -> tuple[MerkleDeploymentScenarioScore, ...]:
    """Export the multi-scenario breakdown of the deployment ranking.

    The explanatory counterpart of
    :func:`recommend_merkle_deployment_weighted_scenarios`: it computes
    :func:`merkle_deployment_frontier` exactly once and, instead of
    returning only the winning profile, returns one frozen
    :class:`MerkleDeploymentScenarioScore` row per frontier member, in
    ...
    (:attr:`MerkleStorageProfile.proof_wire_bytes`) and the per-signature
    verifier hash-chain step count (``profile("merkle", ...)``'s
    ``steps``) — the per-scenario weighted scores, the per-scenario
    ...
    Every weight must be a non-boolean, non-negative integer and at least
    one weight of each scenario must be positive; repeated scenarios are
    counted separately. Each of the four costs is min-max normalised over
    the whole frontier as ``(x - min) / (max - min)``, with a zero span
    scoring ``0``. For each scenario, a candidate's score is the weighted
    sum of its four normalised costs divided by the scenario's weight
    total, and its regret is that score minus the scenario's best score
    over the whole frontier — all exact rationals
    (``fractions.Fraction``) with no floating point anywhere. Exactly one
    row is selected: the one whose config
    :func:`recommend_merkle_deployment_weighted_scenarios` returns for
    the same arguments, field for field — the smallest worst regret,
    with ties broken by the regret sum, then the per-scenario score
    tuple, then ascending by checkpoint bytes, leaf count, ``w`` and
    ``height``.
    ...
    for storage, row in zip(frontier, metric_rows):
        ...
        entries.append((storage, costs, scores))
    ...
    entries = [
        (storage, costs, scores, tuple(
            score - best for score, best in zip(scores, scenario_best)
        ))
        for storage, costs, scores in entries
    ]
    ...
    def ranking(
        ...
        storage, _costs, scores, regrets = entry
        return (
            max(regrets),
            sum(regrets, Fraction(0)),
            scores,
    ...
    return tuple(
        MerkleDeploymentScenarioScore(
            storage, *costs, scores, regrets, storage is chosen
        )
        for storage, costs, scores, regrets in entries
    )
...
@dataclass(frozen=True)
class MerkleTransportDeploymentScenarioScore:
    """Frozen multi-scenario breakdown for one joint Merkle deployment.

    One row of
    :func:`explain_merkle_transport_deployment_weighted_scenarios`'s
    result. Fields, in positional order:
    ...
    - ``regrets``: the per-scenario regrets, in the same order — each
      score minus that scenario's best score over the whole frontier;
    - ``selected``: ``True`` on exactly the one row
      :func:`recommend_merkle_transport_deployment_weighted_scenarios`
      would pick.
...
def explain_merkle_transport_deployment_weighted_scenarios(
    capacity: Any,
    indices: Any,
    budgets: Any,
    scenarios: Any,
) -> tuple[MerkleTransportDeploymentScenarioScore, ...]:
    """Export the multi-scenario breakdown of the joint-deployment ranking.

    The explanatory counterpart of
    :func:`recommend_merkle_transport_deployment_weighted_scenarios`: it
    computes :func:`merkle_transport_deployment_frontier` exactly once
    and, instead of returning only the winning profile, returns one frozen
    :class:`MerkleTransportDeploymentScenarioScore` row per frontier
    member, in the frontier's own order. Each row carries the candidate's
    :class:`MerkleTransportDeploymentProfile` deployment, its five
    min-max-normalised costs in the same order as the weights — the
    batch-proof wire bytes (:attr:`MerkleTransportDeploymentProfile.batch`),
    the multi-proof wire bytes
    ...
    the checkpoint bytes (:attr:`MerkleStorageProfile.checkpoint_bytes`)
    and the per-signature verifier hash-chain step count
    (``profile("merkle", ...)``'s ``steps``) — the per-scenario weighted
    scores, the per-scenario regrets and a ``selected`` flag.

    ``scenarios`` must be a non-empty tuple; each member must itself be a
    five-tuple in that same order — batch-proof bytes, multi-proof bytes,
    carried nodes, checkpoint bytes and per-signature chain steps. Every
    ...
    counted separately. Each of the five costs is min-max normalised over
    the whole frontier as ``(x - min) / (max - min)``, with a zero span
    scoring ``0``. For each scenario, a candidate's score is the weighted
    sum of its five normalised costs divided by the scenario's weight
    total, and its regret is that score minus the scenario's best score
    over the whole frontier — all exact rationals
    (``fractions.Fraction``) with no floating point anywhere. Exactly one
    row is selected: the one whose deployment
    :func:`recommend_merkle_transport_deployment_weighted_scenarios`
    returns for the same arguments, field for field — the smallest worst
    regret, with ties broken by the regret sum, then the per-scenario
    score tuple, then ascending by checkpoint bytes, leaf count, ``w``
    and ``height``.
    ...
    inclusive-budget, exception and no-feasible-candidate rules exactly,
    so a violation raises exactly as that function does (and is screened
    before ``scenarios``). A non-tuple ``scenarios`` or scenario member,
    or a non-integer weight, raises ``TypeError``; an empty scenario
    tuple, a wrong-length scenario, or a boolean, negative or all-zero
    weight raises ``ValueError``. The function is pure: it draws no
    randomness, generates no keys and changes no state.
    """
    frontier = merkle_transport_deployment_frontier(capacity, indices, budgets)
    ...
    return tuple(
        MerkleTransportDeploymentScenarioScore(
            deployment, *costs, scores, regrets, deployment is chosen
        )
...
@dataclass(frozen=True)
class MerkleTransportWorkloadScore:
    """Frozen decision-cost breakdown for one workload deployment.

    One row of :func:`explain_merkle_transport_workload_weighted`'s
    result. Fields, in positional order:
    ...
    - ``score``: the final weighted score — the four normalised costs
      times their weights, summed and divided by the weight total;
    - ``selected``: ``True`` on exactly the one row
      :func:`recommend_merkle_transport_workload_weighted` would pick.
...
def explain_merkle_transport_workload_weighted(
    capacity: Any,
    groups: Any,
    budgets: Any,
    weights: Any,
) -> tuple[MerkleTransportWorkloadScore, ...]:
    """Export the decision-cost breakdown of the weighted workload ranking.

    The explanatory counterpart of
    :func:`recommend_merkle_transport_workload_weighted`: it computes
    :func:`merkle_transport_workload_frontier` exactly once and, instead
    of returning only the winning profile, returns one frozen
    :class:`MerkleTransportWorkloadScore` row per frontier member, in
    the frontier's own order. Each row carries the candidate's
    :class:`MerkleTransportWorkloadProfile` workload plan, its four
    min-max-normalised costs in the same order as the weights — the
    checkpoint bytes (:attr:`MerkleStorageProfile.checkpoint_bytes`), any
    ...
    ``weights`` must be a four-tuple in that same order — checkpoint
    bytes, single-group peak, total transport bytes and per-signature
    chain steps. Every weight must be a non-boolean, non-negative
    integer and at least one must be positive. Each of the four costs
    ...
    :func:`recommend_merkle_transport_workload_weighted` returns for
    the same arguments, field for field — the smallest score, with
    equal scores broken ascending by checkpoint bytes, leaf count,
    ``w``, ``height`` and the per-group ``modes`` tuple in
    lexicographic order. When every span is zero every row scores ``0``
    and the tie-break tail alone decides; the rows report exactly that.

    ``capacity``, ``groups`` and the four-tuple ``budgets`` follow
    :func:`merkle_transport_workload_frontier`'s types, ranges,
    inclusive-budget, exception and no-feasible-candidate rules exactly,
    so a violation raises exactly as that function does (and is screened
    before ``weights``). A non-tuple ``weights`` or a non-integer member
    raises ``TypeError``; a wrong-length tuple or a boolean, negative or
    all-zero weight raises ``ValueError``. The function is pure: it
    draws no randomness, generates no keys and changes no state.
    """
    frontier = merkle_transport_workload_frontier(capacity, groups, budgets)
    ...
    def ranking(
        entry: tuple[MerkleTransportWorkloadProfile, tuple[Fraction, ...], Fraction],
    ) -> tuple[Fraction, int, int, int, int, tuple[str, ...]]:
        workload, _costs, score = entry
        config = workload.config
        return (
            score,
            config.checkpoint_bytes,
...
def explain_merkle_transport_workload_weighted_scenarios(
    ...
    computes :func:`merkle_transport_workload_frontier` exactly once and,
    instead of returning only the winning profile, returns one frozen
    :class:`MerkleTransportWorkloadScenarioScore` row per frontier
    member, in the frontier's own order. Each row carries the candidate's
    :class:`MerkleTransportWorkloadProfile` workload plan, its four
    min-max-normalised costs in the same order as the weights — the
    checkpoint bytes (:attr:`MerkleStorageProfile.checkpoint_bytes`), any
    ...
    ``scenarios`` must be a non-empty tuple; each member must itself be a
    four-tuple in that same order — checkpoint bytes, single-group peak,
    total transport bytes and per-signature chain steps. Every weight
    must be a non-boolean, non-negative integer and at least one weight
    of each scenario must be positive; repeated scenarios are counted
    separately. Each of the four costs is min-max normalised over the
    whole frontier as ``(x - min) / (max - min)``, with a zero span
    scoring ``0``. For each scenario, a candidate's score is the
    weighted sum of its four normalised costs divided by the scenario's
    weight total, and its regret is that score minus the scenario's best
    score over the whole frontier — all exact rationals
    (``fractions.Fraction``) with no floating point anywhere. Exactly
    one row is selected: the one whose workload plan
    :func:`recommend_merkle_transport_workload_weighted_scenarios`
...
@dataclass(frozen=True)
class MerkleModeScore:
    """Frozen decision-cost breakdown for one per-group mode plan.

    One row of :func:`explain_merkle_mode_weighted`'s result. Fields,
    in positional order:

    - ``workload``: the candidate's :class:`MerkleTransportWorkloadProfile`
      mode plan;
    - ``checkpoint_cost`` / ``peak_cost`` / ``transport_cost`` /
      ``steps_cost`` / ``nodes_cost``: the candidate's five
      min-max-normalised costs, in the same order as the weights —
      checkpoint bytes, any single group's transport peak, aggregate
      transport bytes, per-signature verifier chain steps and carried
      multi-proof nodes — each ``(x - min) / (max - min)`` over the whole
      frontier, with a zero span scoring ``0``;
    - ``score``: the final weighted score — the five normalised costs
      times their weights, summed and divided by the weight total;
    - ``selected``: ``True`` on exactly the one row
      :func:`recommend_merkle_mode_weighted` would pick.
...
def explain_merkle_mode_weighted(
    capacity: Any,
    groups: Any,
    budgets: Any,
    weights: Any,
) -> tuple[MerkleModeScore, ...]:
    """Export the decision-cost breakdown of the weighted mode ranking.

    The explanatory counterpart of :func:`recommend_merkle_mode_weighted`:
    it computes :func:`merkle_mode_frontier` exactly once and, instead of
    returning only the winning plan, returns one frozen
    :class:`MerkleModeScore` row per frontier member, in the frontier's
    own order. Each row carries the candidate's
    :class:`MerkleTransportWorkloadProfile` mode plan, its five
    min-max-normalised costs in the same order as the weights — the
    checkpoint bytes (:attr:`MerkleStorageProfile.checkpoint_bytes`), any
    single group's transport peak (the maximum of
    :attr:`MerkleTransportWorkloadProfile.sizes`), the aggregate transport
    bytes (:attr:`MerkleTransportWorkloadProfile.total`), the
    per-signature verifier hash-chain step count (``profile("merkle",
    ...)``'s ``steps``) and the total carried multi-proof node count,
    using the mode frontier's definition: it accumulates the canonical
    node count of every group carried as a multi-proof and counts ``0``
    for every group carried as a batch proof — the final weighted score
    and a ``selected`` flag.

    ``weights`` must be a five-tuple in that same order — checkpoint
    bytes, single-group peak, total transport bytes, per-signature chain
    steps and carried nodes. Every weight must be a non-boolean,
    non-negative integer and at least one must be positive. Each of the
    five costs is min-max normalised over the whole frontier as
    ``(x - min) / (max - min)``, with a zero span scoring ``0``; the
    score is the sum of the five normalised costs times their weights,
    divided by the weight total, all exact rationals
    (``fractions.Fraction``) with no floating point anywhere. Exactly
    one row is selected: the one whose plan
    :func:`recommend_merkle_mode_weighted` returns for the same
    arguments, field for field — the smallest score, with equal scores
    broken ascending by checkpoint bytes, leaf count, ``w``, ``height``
    and the ``modes`` tuple in lexicographic order, exactly like every
    other mode ranking. When every span is zero every row scores ``0``
    and the tie-break tail alone decides; the rows report exactly that.
    ...
    chosen = min(entries, key=ranking)[0]
    return tuple(
        MerkleModeScore(workload, *costs, score, workload is chosen)
        for workload, costs, score in entries
    )


@dataclass(frozen=True)
class MerkleModeScenarioScore:
    """Frozen multi-scenario breakdown for one per-group mode plan.

    One row of :func:`explain_merkle_mode_weighted_scenarios`'s result.
    Fields, in positional order:
    ...
    - ``regrets``: the per-scenario regrets, in the same order — each
      score minus that scenario's best score over the whole frontier;
    - ``selected``: ``True`` on exactly the one row
      :func:`recommend_merkle_mode_weighted_scenarios` would pick.
...
def explain_merkle_mode_weighted_scenarios(
    capacity: Any,
    groups: Any,
    budgets: Any,
    scenarios: Any,
) -> tuple[MerkleModeScenarioScore, ...]:
    """Export the multi-scenario breakdown of the weighted mode ranking.

    The explanatory counterpart of
    :func:`recommend_merkle_mode_weighted_scenarios`: it computes
    :func:`merkle_mode_frontier` exactly once and, instead of returning
    only the winning plan, returns one frozen
    :class:`MerkleModeScenarioScore` row per frontier member, in the
    frontier's own order. Each row carries the candidate's
    :class:`MerkleTransportWorkloadProfile` mode plan, its five
    min-max-normalised costs in the same order as the weights — the
    checkpoint bytes (:attr:`MerkleStorageProfile.checkpoint_bytes`), any
    ...
    using the mode frontier's definition: it accumulates the canonical
    node count of every group carried as a multi-proof and counts ``0``
    for every group carried as a batch proof — the per-scenario weighted
    scores, the per-scenario regrets and a ``selected`` flag.
    ...
    Every weight must be a non-boolean, non-negative integer and at least
    one weight of each scenario must be positive; repeated scenarios are
    counted separately, exactly like repeated groups. Each of the five
    ...
    plan :func:`recommend_merkle_mode_weighted_scenarios` returns for the
    same arguments, field for field — the smallest worst regret, with
    ties broken by the regret sum, then the per-scenario score tuple,
...
@dataclass(frozen=True)
class MerkleVerifyModeDeploymentScore:
    """Frozen decision-cost breakdown for one fixed-position mode plan.

    One row of :func:`explain_merkle_verify_mode_deployment_weighted`'s
    result. Fields, in positional order:
    ...
    - ``score``: the final weighted score — the five normalised costs
      times their weights, summed and divided by the weight total;
    - ``selected``: ``True`` on exactly the one row
      :func:`recommend_merkle_verify_mode_deployment_weighted` would pick.
...
def explain_merkle_verify_mode_deployment_weighted(
    capacity: Any,
    groups: Any,
    budgets: Any,
    weights: Any,
) -> tuple[MerkleVerifyModeDeploymentScore, ...]:
    """Export the decision-cost breakdown of the weighted verify-mode ranking.

    The explanatory counterpart of
    :func:`recommend_merkle_verify_mode_deployment_weighted`: it computes
    :func:`merkle_verify_mode_frontier` exactly once and, instead of
    returning only the winning plan, returns one frozen
    :class:`MerkleVerifyModeDeploymentScore` row per frontier member, in
    the frontier's own order. Each row carries the candidate's
    ...
    and at least one must be positive. Each of the five costs is min-max
    normalised over the whole frontier as ``(x - min) / (max - min)``, with
    a zero span scoring ``0``; the score is the sum of the five normalised
    costs times their weights, divided by the weight total, all exact
    rationals (``fractions.Fraction``) with no floating point anywhere.
    Exactly one row is selected: the one whose plan
    :func:`recommend_merkle_verify_mode_deployment_weighted` returns for
    the same arguments, field for field — the smallest score, with equal
    scores broken ascending by checkpoint bytes, leaf count, ``w``,
    ``height`` and the ``modes`` tuple in lexicographic order. When every
    span is zero every row scores ``0`` and the tie-break tail alone
    decides; the rows report exactly that.
