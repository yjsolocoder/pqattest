...
# 多组工作负载多情景推荐的决策成本明细：每个前沿候选一行，行序与前沿
# 一致，依次为工作负载方案、四项归一化成本（检查点、单组峰值、总传输、
# 链步）、各情景得分元组、各情景后悔值元组与选中标志；恰好一行
# selected=True，其方案与
# recommend_merkle_transport_workload_weighted_scenarios 同参数的结果
# 逐字段相同
explain_merkle_transport_workload_weighted_scenarios(
    16,
    ((0, 1), (3, 5), (8, 9, 10)),
