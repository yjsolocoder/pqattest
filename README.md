...
- `advance_merkle_auth_state(data, next_index, *, key, min_generation=None, claim) -> ((before, after), envelope, generation)` — **无隐藏状态**的「认证恢复 + 作废叶 + 下一代封装」转换，是 `MerkleSigner.advance_to_with_auth_state` 的无状态对应物：`data` 为 `"merkle"` 的 v2 封装（`bytes`/`bytearray`），先按既有 v2 规则用 `hmac.compare_digest` 验 HMAC，再固定方案为 `"merkle"`、应用代次下限，**最后**才从原样 v1 检查点恢复并按 `advance_to` 语义把 `next_index` 推进到目标，随后把推进后的 v1 检查点以 `scheme="merkle"`、原 `key` 与 **g+1** 代次封装。返回 `((before, after), envelope, generation)`：内层二元组为推进前后的下一可用叶索引（原索引与目标，与同目标下 `advance_to` 的返回相同），`envelope` 与对推进后检查点直接调用 `auth_state_wrap` **逐字节相同**，`generation` 为 `g+1`。**等值目标合法**：状态不移动，仍产出认证该状态的下一代封装；推到叶总数即作签署尽状态。`data`/`key` 仅收 `bytes`/`bytearray` 且 `key` 非空；`next_index` 须为闭区间 `[恢复签名器当前 next_index, public_key.leaf_count]` 内的非布尔整数；`min_generation` 为 `None` 或非布尔 uint64；`claim` 须可调用。入参 `g` 必须小于 `2**64-1`，否则抛 `ValueError`。**全部输出生成后**才以唯一入参 `(("merkle", g), ("merkle", g+1))` 恰好调用一次 `claim`，仅返回值 `is True` 时成功（否则抛 `ValueError`），回调异常原样透传；空 key、认证失败、方案不符、代次低于下限或触顶、目标倒退/越界、检查点非法或认领拒绝等任何先前失败都**不调用** `claim` 且无部分返回。错型（`data`/`key` 非 `bytes`/`bytearray`、布尔或非整数目标/下限、`claim` 不可调用）抛 `TypeError`，其余前置拒绝抛 `ValueError`。不修改任何对象、不保留库内状态、不取随机数、不新增线格式，同一输入重复调用逐字节相同；封装仅认证不加密，本身不防重放/回滚
...
只作废叶子（恢复后跳过可能已暴露的叶、不签名）则用 `advance_merkle_auth_state`：它把恢复签名器的 `next_index` 按 `advance_to` 语义推进到目标并封装到 `g+1`，认领令牌同样是一对 `(g, g+1)`；等值目标也合法，仍产出下一代封装：

```python
from pqattest import advance_merkle_auth_state

(before, after), next_blob, generation = advance_merkle_auth_state(
    blob, 3, key=key,
    min_generation=state["high_water"], claim=claim_transition,
)
assert (before, after) == (0, 3)
assert generation == 8
scheme, _, checkpoint = auth_state_unwrap(next_blob, key=key, expect="merkle")
assert MerkleSigner.from_checkpoint(checkpoint).next_index == 3
```
