...
# 单叶集合联合部署多情景推荐的决策成本明细：每个前沿候选一行，行序与前沿
# 一致，依次为部署方案、五项归一化成本（批次证明、多证明、节点数、检查点、
