...
class SignMerkleAuthStateTest(unittest.TestCase):
    ...
    def test_signs_current_minimum_leaf_of_restored_state(self):
        signer = make_signer()
        signer.sign(b"a")
        signer.sign(b"b")
        ...
        reference.sign(b"b")
        signature, _, generation = sign_merkle_auth_state(
            envelope, b"c", key=KEY, claim=accept
        )
        self.assertEqual(signature, reference.sign(b"c"))
    ...
    def test_stateless_chaining_through_envelopes(self):
        ...
        for leaf in range(4):
            signature, envelope, generation = sign_merkle_auth_state(
                envelope, f"m{leaf}", key=KEY, claim=accept
            )
            self.assertEqual(signature.index, leaf)
            self.assertEqual(generation, leaf + 1)
    ...
    def test_stateless_same_envelope_replays_identically(self):
        first = sign_merkle_auth_state(
            self.envelope, b"m", key=KEY, claim=accept
        )
        second = sign_merkle_auth_state(
            self.envelope, b"m", key=KEY, claim=accept
        )
    ...
    def test_does_not_mutate_state_behind_input_envelope(self):
        ...
        sign_merkle_auth_state(envelope, b"m", key=KEY, claim=accept)
        self.assertEqual(signer.next_index, 0)

    def test_floor_equal_and_below_pass(self):
        envelope = wrap_merkle(self.signer, generation=7)
        sign_merkle_auth_state(envelope, b"m", key=KEY, min_generation=7,
                               claim=accept)
        sign_merkle_auth_state(envelope, b"m", key=KEY, min_generation=6,
                               claim=accept)
...
class SignMerkleAuthStateTypesTest(unittest.TestCase):
    ...
    def test_keyword_only_arguments(self):
        with self.assertRaises(TypeError):
            sign_merkle_auth_state(self.envelope, b"m", KEY, None, accept)
        with self.assertRaises(TypeError):
            sign_merkle_auth_state(self.envelope, b"m", KEY, claim=accept)
    ...
    def test_non_bytes_data_type_error(self):
        for bad in (None, 42, 4.5, "blob", [self.envelope], object()):
            with self.subTest(bad=type(bad).__name__):
                with self.assertRaises(TypeError):
                    sign_merkle_auth_state(bad, b"m", key=KEY, claim=accept)
    ...
    def test_types_checked_before_envelope(self):
        with self.assertRaises(TypeError):
            sign_merkle_auth_state(b"", b"m", key=42, claim=accept)
        with self.assertRaises(TypeError):
            sign_merkle_auth_state(object(), b"m", key=KEY, claim=accept)
        with self.assertRaises(TypeError):
            sign_merkle_auth_state(
        ...
        with self.assertRaises(TypeError):
            sign_merkle_auth_state(b"", 123, key=KEY, claim=accept)
