...
- `recommend_merkle_deployment(capacity, budgets, prefer="size")` — 纯函数：在预算约束下选可行 Merkle 配置并返回其 `MerkleStorageProfile`。`capacity` 限 1 至 256 非布尔整数；`budgets` 为四元组，依次限制检查点字节、签名线长、独立证明线长与验签链步数，各项为 `None` 或正的非布尔整数且至少一项非空。枚举 `w ∈ {4, 8}` 与 `height ∈ 1..8` 中容量可覆盖（`2**height >= capacity`）且满足全部非空预算的候选；`prefer="size"` 依次最小化签名线长、证明线长、检查点、步数、叶数、`w`、`height`，`prefer="speed"` 先最小化步数再沿用其余顺序，取排序首项。`budgets` 非元组抛 `TypeError`；成员、`capacity`、`prefer` 非法或无可行候选抛 `ValueError`
