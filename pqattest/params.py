...
def recommend_merkle_mode_weighted_scenarios(
    ...
    """Pick one per-group mode choice minimising worst regret over scenarios.

    The preference-drift-robust counterpart of
    :func:`recommend_merkle_mode_deployment`: instead of trusting one
    business preference, it evaluates several weight scenarios at once and
    picks the member of :func:`merkle_mode_frontier`'s returned per-group
    mode frontier with the smallest worst-case regret — the maximum, over
    scenarios, of how much worse the chosen mode combination scores than
    the best combination for that scenario. The five weighted costs are
    the mode frontier's own five, in its own order: the checkpoint bytes
    ...
    per-signature verifier hash-chain steps (``profile("merkle", ...)``'s
    ``steps``) and the carried multi-proof node total — which accumulates
    the canonical multi-proof node count of every group carried as a
    multi-proof and counts ``0`` for every group carried as a batch proof,
    exactly like the mode frontier.

    ``scenarios`` must be a non-empty tuple; each member must itself be a
    five-tuple in the order checkpoint bytes, single-group peak, aggregate
    transport bytes, per-signature chain steps and carried node total.
    ...
    counted separately, exactly like repeated groups.
    ...
    duplicated. Each of the five costs is min-max normalised over the
    whole frontier as ``(x - min) / (max - min)``, with a zero span
    scoring ``0``. For each scenario, a candidate's score is the weighted
    sum of its five normalised costs divided by the scenario's weight
    total, all compared as exact rationals (``fractions.Fraction``) with
    no floating point anywhere. Each scenario's own minimum score is then
    subtracted from every candidate's score to give that scenario's
    regret; combinations are ranked first by the greatest regret over the
    scenarios (the minimax-regret choice), then by the sum of the regrets,
    then by the tuple of per-scenario scores, all ascending. Every ranking
    finishes with the same ascending tie-break as every other mode
    ranking — checkpoint bytes, leaf count, ``w``, ``height`` and the
    ``modes`` tuple in lexicographic order — and the first survivor is
    returned as a :class:`MerkleTransportWorkloadProfile`.
