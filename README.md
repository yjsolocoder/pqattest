...
- `recommend_merkle_verify_mode_weighted(capacity, groups, budgets, scenarios)` — 在 `merkle_verify_mode_frontier` 的非支配结果上**同时评估多组五元组权重情景，选出最坏后悔值最小的固定叶位置模式方案**，返回该前沿成员（现有的 `MerkleModeCost`），不新增值类型、不复制候选枚举与 Pareto 筛选、不改变任何线格式。纯函数：不取随机数、不生成密钥、不改状态；前沿在函数内恰好调用一次。四个参数均无默认值；`capacity`、`groups` 与六元组 `budgets` 完全沿用 `merkle_verify_mode_frontier` 的类型、范围、含边界预算、异常与无可行项规则，并先于 `scenarios` 校验。`scenarios` 必须为非空元组，每项是五元组非布尔非负整数权重，依次对应总传输（`plan.total`）、单组峰值（`max(plan.sizes)`）、验签 SHA-256 总量（`cost.total`）、multiproof 节点数（`nodes`）与单签链步（`profile("merkle", ...).steps`），每项至少一个正数，重复情景分别计入。对前沿五项成本分别按 `(x-min)/(max-min)` 归一化为 `Fraction`（零跨度记 0），每个情景的加权和除以该情景权重总和，全程无浮点。对每个情景先求其最小归一化得分，再以候选得分减该最小值得到后悔值；选择最大后悔值最小者，再按后悔值总和、各情景得分元组及既有稳定末段键（检查点字节、叶数、`w`、`height`、`modes` 字典序）升序决胜。`scenarios` 或其成员非元组、权重非整数抛 `TypeError`；空、错长、布尔、负数或全零项抛 `ValueError`
...
# 固定叶位置时给若干组五元组权重情景（每组依次对应总传输、单组峰值、
# 验签哈希、节点、链步），在 merkle_verify_mode_frontier 上取最坏后悔值最小者
recommend_merkle_verify_mode_weighted(
