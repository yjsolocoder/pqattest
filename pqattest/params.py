...
def _validate_cardinality_scenarios(scenarios: Any) -> tuple[tuple[int, ...], ...]:
    """Validate the non-empty tuple of five-weight scenarios."""
    if not isinstance(scenarios, tuple):
        raise TypeError("scenarios must be a tuple of five-tuples of weights")
    if not scenarios:
...
def recommend_merkle_cardinality_scenarios(
    capacity: Any,
    group_sizes: Any,
    budgets: Any,
    scenarios: Any,
    ...
    The preference-drift-robust counterpart of
    :func:`recommend_merkle_cardinality_weighted` for the leaf-position-not-
    yet-fixed workload of :func:`merkle_cardinality_frontier`: instead of
    trusting one five-tuple of cost weights, it evaluates several weight
    scenarios at once and picks the frontier member with the smallest
    worst-case regret — the maximum, over scenarios, of how much worse the
    chosen plan scores than the best plan for that scenario. The five
    weighted costs are the same ones
    :func:`recommend_merkle_cardinality_deployment` ranks: aggregate
    ...
    ``scenarios`` must be a non-empty tuple; each member must itself be a
    five-tuple in the order total transport, single-group peak, total
    verifier SHA-256 hashes, carried multi-proof nodes and per-signature
    chain steps. Every weight must be a non-boolean, non-negative integer
    and at least one weight of each scenario must be positive; repeated
    scenarios are counted separately, exactly like repeated groups.
    ...
    other cardinality ranking — checkpoint bytes, leaf count, ``w``,
    ``height`` and the ``modes`` tuple in lexicographic order — and the
    first survivor is returned as a :class:`MerkleModeCost`.

    ``capacity``, ``group_sizes`` and the six-tuple ``budgets`` follow
    :func:`merkle_cardinality_frontier`'s types, ranges,
    ...
    """
    frontier = merkle_cardinality_frontier(capacity, group_sizes, budgets)
    validated_scenarios = _validate_cardinality_scenarios(scenarios)
