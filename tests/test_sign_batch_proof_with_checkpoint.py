...
class SignBatchProofWithCheckpointTest(unittest.TestCase):
    ...
    def test_proof_round_trips_and_verifies_the_signed_messages(self):
        ...
        self.assertTrue(parsed.verify(messages))
        self.assertFalse(parsed.verify(tuple("x" for _ in messages)))
        for message, signature in zip(messages, parsed.signatures):
    ...
    def test_single_message_is_a_valid_one_signature_batch(self):
        signer = make_signer()
    ...
    def test_proof_encoding_failure_leaves_state_unchanged(self):
        ...
        with mock.patch.object(
            pqattest.merkle.MerkleBatchProof,
            "to_bytes",
            side_effect=ValueError("encoding failure"),
        ):
            with self.assertRaises(ValueError):
