...
- `explain_merkle_deployment_weighted(capacity, budgets, weights)` — `recommend_merkle_deployment_weighted` 的**决策成本明细版**：对同一个普通 Merkle 部署前沿用同一组四元权重、同一套 min-max 归一化与同一加权评分逐行打分，但不只返回中选项，而是返回冻结的 `MerkleDeploymentScore` 行元组 `tuple[MerkleDeploymentScore, ...]`，**行序与 `merkle_deployment_frontier` 前沿成员顺序完全一致**，不重复枚举或筛选候选、不改变既有前沿、各族推荐入口、任何旧接口与线格式。纯函数：不取随机数、不生成密钥、不改状态，同一输入重复调用结果逐项相同；前沿在函数内恰好调用一次。三参数均无默认值；`capacity` 与四元组 `budgets` 先按 `merkle_deployment_frontier` 原规则校验（类型、范围与含边界预算规则、异常与无可行项规则完全沿用，且先于 `weights` 筛查），预算下无可行方案时抛 `ValueError` 且不返回任何明细行。`weights` 必须为四元组，依次对应检查点字节、单签线长、独立证明线长、单签验签链步；权重容器非元组或成员非整数抛 `TypeError`；长度不符、含布尔或负数、整组全零抛 `ValueError`。每行依次携带该候选的配置（`MerkleStorageProfile`）、与权重逐位对应的四项归一化成本（每项为精确 `fractions.Fraction`，零跨度记 0）、最终评分（四项归一化成本乘权重求和再除以权重总和，全程 `Fraction` 精确有理数、禁止浮点）以及选中标志。恰好一行 `selected=True`，其配置与同参数调用 `recommend_merkle_deployment_weighted` 的结果逐字段相同；评分完全相同时沿用既有决胜尾序，按检查点字节、叶数、`w`、`height` 升序取首。全前沿零跨度时各行评分均为 0，选中项完全由该决胜尾序决定，明细如实反映
- `MerkleDeploymentScore` — 冻结的加权推荐决策成本明细行值对象，四个字段按位置依次为 `config, costs, score, selected`，类型依次为 `MerkleStorageProfile`、`Fraction` 四元组、`Fraction`、`bool`：候选配置、与权重逐位对齐的四项归一化成本、最终加权评分及该行是否为唯一中选行；冻结、可位置构造、按值相等（可哈希）
...
# 决策成本明细：用同一组权重对整个前沿逐行打分，返回 MerkleDeploymentScore 行
# 元组，行序与 merkle_deployment_frontier 完全一致；每行依次是候选配置、四项
# 归一化成本（Fraction）、最终评分（Fraction）与唯一的选中标志
explain_merkle_deployment_weighted(
    4, (None, None, None, 9000), (1, 1, 1, 1)
)
# (MerkleDeploymentScore(config=MerkleStorageProfile(w=4, height=2, ...),
#                        costs=(1, 1, 1, 0), score=3/4, selected=False),
#  MerkleDeploymentScore(config=MerkleStorageProfile(w=8, height=2, ...),
#                        costs=(0, 0, 0, 1), score=1/4, selected=True))
# 恰好一行 selected=True，其 config 与 recommend_merkle_deployment_weighted
# 的返回逐字段相同；全前沿零跨度时各行 score 均为 0，选中项按检查点、叶数、
# w、height 决胜尾序决定；无可行候选时抛 ValueError 且不返回任何行
