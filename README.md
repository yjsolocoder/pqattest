...
- `MerkleProof.verify_bound(message, *, public_key, index=None)` — 把单份证明绑定到接收方预期公钥与可选叶选择；不新增对象、不改动证明 v1 线格式，不生成密钥、不取随机数、不保存状态。`public_key` 须为 `MerklePublicKey`（错型抛 `TypeError`）；先按 `w`、`height`、`root` 逐值比较预期公钥与包内公钥，再沿用 `merkle_verify` 验证消息与签名，全部匹配才返回 `True`。`index=None` 不限制叶选择；显式值须为非布尔整数（非整数抛 `TypeError`）并等于签名的 `index`；布尔、负数、越界或不相等均返回 `False`。包内公钥或签名字段缺失、错型（含绕过冻结构造器形成的畸形结构），以及消息、签名或公钥值不匹配均返回 `False` 而不泄漏其他异常
...
- `MerkleBatchProof.verify_bound(messages, *, public_key, indices=None)` — 在 `verify(messages)` 的逐项验证之外，把批次绑定到接收方预期公钥与可选叶选择；不新增对象、不改动批次证明 v1 线格式，不生成密钥、不取随机数、不保存状态。外部参数 `public_key` 须为 `MerklePublicKey`（错型抛 `TypeError`）；显式 `indices` 须为元组（非元组或含非整数成员抛 `TypeError`）。包内公钥必须是 `MerklePublicKey`、包内签名必须是非空的 `MerkleSignature` 元组——字段缺失、错型、空元组或非元组（含绕过冻结构造器形成的畸形结构）一律返回 `False`；公钥还须与包内公钥的 `w`、`height`、`root` 逐值相等。`indices=None` 不限制叶选择；显式 `indices` 与消息等长、成员为非布尔整数、严格递增，并与各签名的 `index` 按位置完全相同；布尔成员、重复、乱序、越界、数量不符，以及消息、签名或公钥值不匹配均返回 `False` 而不抛异常；仅外部参数错型沿用抛 `TypeError` 的旧约
