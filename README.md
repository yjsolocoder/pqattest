...
# 同一单权重推荐的决策成本明细：每个前沿候选一行，行序与前沿一致，依次为
# 方案成本配对、五项归一化成本、最终评分与选中标志；恰好一行 selected=True，
# 其方案与 recommend_merkle_verify_mode_deployment_weighted 同参数的结果逐字段相同
explain_merkle_verify_mode_deployment_weighted(
    16,
    ((0, 1), (3, 5)),
    (None, 5000, 12000, None, 8, None),
...
#   transport_cost=Fraction(...), peak_cost=Fraction(...), hashes_cost=Fraction(...),
#   nodes_cost=Fraction(...), steps_cost=Fraction(...), score=Fraction(...),
#   selected=False),
#  ...,
#  MerkleVerifyModeDeploymentScore(mode_cost=MerkleModeCost(...), ..., selected=True))
