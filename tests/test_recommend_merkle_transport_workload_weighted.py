...
class RecommendMerkleTransportWorkloadWeightedTest(unittest.TestCase):
    ...
    def test_peak_weight_uses_largest_group_size(self):
        groups = ((0,), (1, 2), (4,))
        budgets = (9000, None, None, None)
        result = recommend_merkle_transport_workload_weighted(
            8, groups, budgets, (0, 1, 0, 0)
        )
        frontier = merkle_transport_workload_frontier(8, groups, budgets)
        self.assertEqual(
            max(result.sizes),
            min(max(workload.sizes) for workload in frontier),
        )
    ...
    def test_boolean_weights_are_value_error_even_though_int(self):
        for bad in ((True, 0, 0, 0), (0, True, 0, 0), (0, 0, True, 0), (0, 0, 0, True)):
            with self.subTest(bad=bad):
                with self.assertRaises(ValueError):
                    recommend_merkle_transport_workload_weighted(
                        16, _GROUPS, _BUDGETS, bad
