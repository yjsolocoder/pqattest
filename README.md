...
- `recommend_merkle_cardinality_deployment(capacity, group_sizes, budgets, prefer="compact")` — 在 `merkle_cardinality_frontier` 的非支配结果上**仅调用一次该前沿**并按业务偏好选出一个方案，返回现有 `MerkleModeCost`（其必为前沿成员），不复制候选枚举与 Pareto 筛选、不新增值类型、不改变任何线格式。纯函数：不取随机数、不生成密钥、不改状态。前三参数无默认值；`capacity`、`group_sizes` 与六元组 `budgets` 完全沿用 `merkle_cardinality_frontier` 的类型、范围、含边界预算、异常与无解规则，无可行项抛 `ValueError`。`prefer` 仅取 `"compact"`（默认）、`"verify"`、`"nodes"`、`"speed"`、`"robust"`，其他值抛 `ValueError`。五项参与排序的成本依次为总传输字节（`plan.total`）、单组峰值（`max(plan.sizes)`）、验签哈希总量（`cost.total`）、节点总数（`nodes`）、单签链步（`profile("merkle", ...).steps`）。`"compact"` 依次最小化总传输、单组峰值、验签哈希总量、节点数、单签链步；`"verify"` 依次最小化验签哈希总量、单签链步、总传输、单组峰值、节点数；`"nodes"` 依次最小化节点数、总传输、验签哈希总量、单组峰值、单签链步；`"speed"` 依次最小化单签链步、验签哈希总量、总传输、单组峰值、节点数；`"robust"` 在前沿五项成本上取各项 `(x-min)/(max-min)` 的**精确有理数**，零跨度（`max==min`）记 0，先最小化最大归一化成本、再最小化归一化成本之和。全部策略末段统一按检查点字节、叶数、`w`、`height`、`modes` 字典序升序取首项。`group_sizes`/`budgets` 非元组、或 `group_sizes` 含非整数成员抛 `TypeError`；其余非法输入抛 `ValueError`
