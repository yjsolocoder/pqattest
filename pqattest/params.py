...
decision-cost breakdown behind that same multi-scenario ranking as a tuple
of frozen :class:`MerkleCardinalityScenarioScore` rows — one per frontier
member, in frontier order, each carrying the candidate's plan/cost
pairing, its five normalised costs, its per-scenario score and regret
tuples and a selected flag — with the same exact
:class:`fractions.Fraction` arithmetic.
...
class MerkleCardinalityScenarioScore:
    ...
    One row of
    :func:`explain_merkle_cardinality_weighted_scenarios`'s result. Fields,
    in positional order:
...
def explain_merkle_cardinality_weighted_scenarios(
    ...
    :func:`merkle_cardinality_frontier` exactly once and, instead of
    returning only the winning plan, returns one frozen
    :class:`MerkleCardinalityScenarioScore` row per frontier member, in the
    frontier's own order. Each row carries the candidate's
    :class:`MerkleModeCost` plan/cost pairing, its five min-max-normalised
    costs in the same order as the weights — aggregate transport bytes
    (:attr:`MerkleTransportWorkloadProfile.total`), any single group's
    ...
    tuple, then ascending by checkpoint bytes, leaf count, ``w``,
