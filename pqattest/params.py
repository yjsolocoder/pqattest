...
workload plan, its four normalised costs, its final score and a selected
flag — with the same exact :class:`fractions.Fraction` arithmetic.
...
class MerkleTransportWorkloadScore:
    """Frozen decision-cost breakdown for one multi-group workload plan.
...
def explain_merkle_transport_workload_weighted(
    ...
    of returning only the winning plan, returns one frozen
    :class:`MerkleTransportWorkloadScore` row per frontier member, in the
    frontier's own order. Each row carries the candidate's
    ...
    :func:`recommend_merkle_transport_workload_weighted` returns for the
    same arguments, field for field — the smallest score, with equal
    scores broken ascending by checkpoint bytes, leaf count, ``w``,
    ``height`` and the per-group ``modes`` tuple in lexicographic order.
    When every span is zero every row scores ``0`` and the tie-break
    tail alone decides; the rows report exactly that.
    ...
    return tuple(
        MerkleTransportWorkloadScore(
            workload, *costs, score, workload is chosen
        )
