...
advance_and_sign_merkle_auth_state combines that jump with a single
signature: it restores the same way, advances to a caller-chosen target leaf
with advance_to semantics, signs that leaf, and wraps the post-signature
checkpoint (target + 1) at generation g+1, returning ((before, target),
signature, envelope, g+1) under the same single paired claim — the target
must stay signable, so its interval ends at the leaf count minus one.
...
def advance_and_sign_merkle_auth_state(
    ...
    """Restore a Merkle v2 envelope, jump to ``next_index``, sign it, and wrap.

    The stateless combination of :func:`advance_merkle_auth_state` and
    :func:`sign_merkle_auth_state`: in one call, with no object mutation,
    hidden library state or new wire format, it performs an authenticated v2
    restore exactly like :meth:`MerkleSigner.from_auth_state`, advances the
    restored signer's next-leaf index to ``next_index`` with
    :meth:`MerkleSigner.advance_to` semantics, signs ``message`` on that
    target leaf exactly like :meth:`MerkleSigner.sign`, and wraps the
    post-signature checkpoint (whose next index is ``next_index + 1``) with
    :func:`auth_state_wrap` fixed to ``scheme="merkle"``, the original
    ``key`` and generation ``g + 1``. ``key``, ``min_generation`` and
    ``claim`` are keyword-only; only ``min_generation`` has a default
    (``None``, no floor). Returns
    ``((before, target), signature, envelope, generation)``: the inner tuple
    is the next-leaf index before the jump and the signed target, the
    :class:`MerkleSignature` for the target leaf, the
    :func:`auth_state_wrap` v2 envelope (``bytes``) over the post-signature
    v1 :meth:`MerkleSigner.checkpoint` bytes — byte-for-byte identical to
    calling :func:`auth_state_wrap` on the checkpoint a signer advanced to
    ``next_index`` and then signed returns from :meth:`checkpoint` — and the
    new generation ``g + 1``. An equal target is legal: the index need not
    move, yet the signature consumes the leaf and the generation still
    advances.

    Apart from ``next_index``, every parameter keeps the type, default and
    error behaviour of :func:`sign_merkle_auth_state`: ``data`` must be
    ``bytes`` or ``bytearray`` holding a v2 envelope; ``key`` must be a
    non-empty ``bytes``/``bytearray`` shared secret; ``message`` follows the
    usual ``bytes``/``bytearray``/``str`` (UTF-8) rules; ``min_generation``
    must be ``None`` or a non-boolean integer in ``0 .. 2**64 - 1``;
    ``claim`` must be callable. ``next_index`` must be a non-boolean
    integer in the closed interval
    ``[restored current index, leaf count - 1]`` — the upper bound is one
    below the leaf count because the target leaf must remain signable and
    the post-signature next index is ``next_index + 1``. A wrong type
    (including a boolean target) raises ``TypeError``. The v2 HMAC tag is
    ...
    restored, and the target is range-checked — authentication or floor
    failure, a target below the restored current index or above the
    signable range, or an input generation ``g`` at the uint64 ceiling (it
    must be strictly below ``2**64 - 1`` so ``g + 1`` fits) raises
    ``ValueError``; a restored state with no signable leaf raises
    :class:`KeyExhaustedError`. The jump, signature and the candidate
    post-signature checkpoint/envelope are built in one critical section,
    and only once every output exists is ``claim`` called exactly once with
    the paired token ``(("merkle", g), ("merkle", g + 1))``; the call
    succeeds only when that return value ``is True`` — otherwise it raises
    ...
    next_generation = generation + 1
    signer = MerkleSigner.from_checkpoint(checkpoint)
    # Build every output before the claim: an out-of-range or unsignable
    # target, an exhausted state or any failure here must leave no partial
    # result and must not call claim.
    with signer._lock:
        before = signer._next_index
        leaf_count = len(signer._private_keys)
        ...
        if next_index < before or next_index > leaf_count - 1:
            raise ValueError(
                "next_index must be between the current index and the leaf count minus one"
            )
