...
class SignMultiproofMerkleAuthStateTest(unittest.TestCase):
    ...
    def test_returns_proof_envelope_and_generation(self):
        result = sign_multiproof_merkle_auth_state(
            self.envelope, (b"a", b"b"), key=KEY, claim=accept
        )
    ...
    def test_proof_matches_sign_batch_then_multiproof_encode(self):
        ...
        expected = multiproof_encode(reference.public_key, signatures)
        self.assertEqual(proof, expected)
        self.assertEqual(generation, 12)
        self.assertTrue(multiproof_verify(MESSAGES, proof))
        self.assertFalse(multiproof_verify((b"m0", b"m1", b"other"), proof))
    ...
    def test_explicit_indices_void_gaps_and_advance_past_last(self):
        signer = make_signer(height=3)  # 8 leaves
        envelope = wrap_merkle(signer, generation=3)
        reference = make_signer(height=3)
        messages = (b"a", b"b", b"c")
        proof, out_envelope, generation = sign_multiproof_merkle_auth_state(
            envelope, messages, indices=(1, 3, 6), key=KEY, claim=accept
        )
        signatures = tuple(reference._signature_at(i, m) for i, m in
                           zip((1, 3, 6), messages))
        self.assertEqual(
            proof, multiproof_encode(reference.public_key, signatures)
        )
        ...
        self.assertEqual(wrapped_generation, 4)
        restored = MerkleSigner.from_checkpoint(checkpoint)
        self.assertEqual(restored.next_index, 7)

    def test_explicit_indices_match_default_when_contiguous_from_current(self):
        messages = (b"x", b"y")
        default = sign_multiproof_merkle_auth_state(
            self.envelope, messages, key=KEY, claim=accept
        )
        explicit = sign_multiproof_merkle_auth_state(
            self.envelope, messages, indices=(0, 1), key=KEY, claim=accept
        )
        self.assertEqual(default, explicit)

    def test_explicit_indices_from_advanced_state(self):
        signer = make_signer(height=3)
        signer.sign_batch((b"a", b"b"))
        envelope = wrap_merkle(signer, generation=9)
        proof, out_envelope, generation = sign_multiproof_merkle_auth_state(
            envelope, (b"m",), indices=(5,), key=KEY, claim=accept
        )
    ...
    def test_last_leaf_selection_succeeds(self):
    ...
    def test_batch_exactly_filling_remaining_leaves_succeeds(self):
        ...
        proof, _, generation = sign_multiproof_merkle_auth_state(
            wrap_merkle(signer, generation=2),
            (b"a", b"b", b"c", b"d"), key=KEY, claim=accept,
        )
        self.assertTrue(multiproof_verify((b"a", b"b", b"c", b"d"), proof))
    ...
    def test_batch_running_past_last_leaf_raises_key_exhausted_without_claim(self):
        ...
        with self.assertRaises(KeyExhaustedError):
            sign_multiproof_merkle_auth_state(
                wrap_merkle(signer, generation=0),
                (b"a", b"b", b"c", b"d", b"e"), key=KEY, claim=claim,
            )
    ...
    def test_accepts_str_bytearray_members_and_bytearray_inputs(self):
        ...
        proof, envelope, generation = sign_multiproof_merkle_auth_state(
            bytearray(self.envelope), messages,
    ...
    def test_stateless_chaining_through_envelopes(self):
        ...
        proof, envelope, generation = sign_multiproof_merkle_auth_state(
            envelope, (b"m0", b"m1"), key=KEY, claim=accept
        )
        self.assertTrue(multiproof_verify((b"m0", b"m1"), proof))
        self.assertEqual(generation, 1)
        proof, envelope, generation = sign_multiproof_merkle_auth_state(
            envelope, (b"m4", b"m6"), indices=(4, 6), key=KEY, claim=accept
        )
        self.assertTrue(multiproof_verify((b"m4", b"m6"), proof))
    ...
    def test_stateless_same_envelope_replays_identically(self):
        first = sign_multiproof_merkle_auth_state(
            self.envelope, (b"m", b"n"), key=KEY, claim=accept
        )
    ...
    def test_out_of_range_indices_raise_value_error_without_claim(self):
        signer = make_signer(height=3)
        signer.sign(b"a")
        envelope = wrap_merkle(signer, generation=4)
        claim = RecordingClaim()
        for bad in ((0,), (1, 8), (9,), (2, 2), (3, 2)):
            with self.subTest(bad=bad):
                claim.calls.clear()
                with self.assertRaises(ValueError):
                    sign_multiproof_merkle_auth_state(
                        envelope, (b"m",) * len(bad), indices=bad,
                        key=KEY, claim=claim,
                    )
                self.assertEqual(claim.calls, [])

    def test_indices_length_mismatch_raises_value_error_without_claim(self):
        claim = RecordingClaim()
        for bad in ((), (0,), (0, 1, 2)):
            with self.subTest(bad=bad):
                claim.calls.clear()
                with self.assertRaises(ValueError):
                    sign_multiproof_merkle_auth_state(
                        self.envelope, (b"a", b"b"), indices=bad,
                        key=KEY, claim=claim,
                    )
                self.assertEqual(claim.calls, [])
...
class SignMultiproofMerkleAuthStateClaimTest(unittest.TestCase):
    ...
    def test_claim_exception_propagates_untouched(self):
        ...
        with self.assertRaises(Boom):
            sign_multiproof_merkle_auth_state(
                self.envelope, (b"m",), key=KEY, claim=boom
            )
    ...
    def test_claim_not_called_on_tamper(self):
        ...
        with self.assertRaises(ValueError):
            sign_multiproof_merkle_auth_state(
                bytes(forged), (b"m",), key=KEY, claim=claim
            )
        self.assertEqual(claim.calls, [])
    ...
    def test_claim_not_called_on_bad_indices(self):
        ...
        with self.assertRaises(ValueError):
            sign_multiproof_merkle_auth_state(
                self.envelope, (b"m",), indices=(4,), key=KEY, claim=claim
            )
        self.assertEqual(claim.calls, [])
...
class SignMultiproofMerkleAuthStateTypesTest(unittest.TestCase):
    ...
    def test_non_tuple_indices_type_error(self):
        for bad in ([0], {0}, b"0", "0", 0, 4.5, object()):
            with self.subTest(bad=type(bad).__name__):
                with self.assertRaises(TypeError):
                    sign_multiproof_merkle_auth_state(
                        self.envelope, (b"m",), indices=bad,
                        key=KEY, claim=accept,
                    )

    def test_non_integer_index_member_type_error(self):
        for bad in ((1.0,), ("1",), (None,), ([1],), (object(),)):
            with self.subTest(bad=bad):
                with self.assertRaises(TypeError):
                    sign_multiproof_merkle_auth_state(
                        self.envelope, (b"m",), indices=bad,
    ...
    def test_boolean_index_member_value_error(self):
