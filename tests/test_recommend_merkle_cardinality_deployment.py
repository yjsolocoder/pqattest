...
def _expected(capacity, group_sizes, budgets, *, prefer):
    """Rank merkle_cardinality_frontier's survivors by the documented preference."""
...
class RecommendMerkleCardinalityDeploymentTest(unittest.TestCase):
    def test_default_prefers_compact(self):
        explicit = recommend_merkle_cardinality_deployment(
            4, _SIZES, _BUDGETS, prefer="compact"
        )
        defaulted = recommend_merkle_cardinality_deployment(4, _SIZES, _BUDGETS)
        self.assertEqual(defaulted, explicit)
        self.assertEqual(
            defaulted, _expected(4, _SIZES, _BUDGETS, prefer="compact")
        )
        ...
        for prefer in _PREFERS:
            for capacity, group_sizes, budgets in workloads:
                with self.subTest(
                    ...
                    result = recommend_merkle_cardinality_deployment(
                        capacity, group_sizes, budgets, prefer=prefer
                    )
                    self.assertEqual(
                        result,
                        _expected(capacity, group_sizes, budgets, prefer=prefer),
                    )
    ...
    def test_returns_mode_cost_on_the_frontier(self):
        ...
        for prefer in _PREFERS:
            with self.subTest(prefer=prefer):
                result = recommend_merkle_cardinality_deployment(
                    4, _SIZES, _BUDGETS, prefer=prefer
                )
    ...
    def test_compact_minimises_total_then_peak(self):
        frontier = merkle_cardinality_frontier(4, _SIZES, _BUDGETS)
        result = recommend_merkle_cardinality_deployment(4, _SIZES, _BUDGETS)
    ...
    def test_nodes_minimises_carried_nodes(self):
        frontier = merkle_cardinality_frontier(
            8, (1, 2, 4), (None, None, None, None, None, 10**18)
        )
        result = recommend_merkle_cardinality_deployment(
            8,
            (1, 2, 4),
    ...
    def test_speed_minimises_steps(self):
        frontier = merkle_cardinality_frontier(4, _SIZES, _BUDGETS)
        result = recommend_merkle_cardinality_deployment(
            4, _SIZES, _BUDGETS, prefer="speed"
        )
        steps = [
    ...
    def test_robust_minimax_normalised_costs_exactly(self):
        capacity, sizes, budgets = 8, (1, 2, 4), (None, None, None, None, None, 10**18)
        frontier = merkle_cardinality_frontier(capacity, sizes, budgets)
        result = recommend_merkle_cardinality_deployment(
            capacity, sizes, budgets, prefer="robust"
        )
        columns = tuple(
    ...
    def test_robust_zero_span_dimension_scores_zero(self):
        ...
        only = recommend_merkle_cardinality_deployment(
            1, (1,), budgets, prefer="robust"
        )
        self.assertEqual(only, frontier[0])

    def test_different_preferences_can_disagree(self):
        capacity, sizes, budgets = 8, (1, 2, 4), (None, None, None, None, None, 10**18)
        compact = recommend_merkle_cardinality_deployment(
            capacity, sizes, budgets, prefer="compact"
        )
        speed = recommend_merkle_cardinality_deployment(
            capacity, sizes, budgets, prefer="speed"
        )
    ...
    def test_budgets_are_honoured(self):
        ...
        for prefer in _PREFERS:
            result = recommend_merkle_cardinality_deployment(
                capacity, sizes, budgets, prefer=prefer
            )
    ...
    def test_no_feasible_candidate_raises(self):
        with self.assertRaises(ValueError):
            recommend_merkle_cardinality_deployment(
                1, (1,), (100, None, None, None, None, None)
            )
        with self.assertRaises(ValueError):
            recommend_merkle_cardinality_deployment(
                1, (257,), (None, None, None, None, None, 1)
            )
        with self.assertRaises(ValueError):
            recommend_merkle_cardinality_deployment(
    ...
    def test_arguments_validated_like_frontier_before_prefer(self):
        ...
        with self.assertRaises(TypeError):
            recommend_merkle_cardinality_deployment(
                4, [1], _BUDGETS, prefer="bogus"
            )

    def test_first_three_arguments_have_no_defaults(self):
        sig = inspect.signature(recommend_merkle_cardinality_deployment)
        self.assertEqual(
            list(sig.parameters),
            ["capacity", "group_sizes", "budgets", "prefer"],
        )
        for name in ("capacity", "group_sizes", "budgets"):
    ...
    def test_pure_no_state_change(self):
        ...
        for prefer in _PREFERS:
            first = recommend_merkle_cardinality_deployment(
                4, (1, 2), (None, None, None, 9000, None, None), prefer=prefer
            )
            second = recommend_merkle_cardinality_deployment(
                4, (1, 2), (None, None, None, 9000, None, None), prefer=prefer
            )
