...
_GROUPS = ((0,), (2, 3))
# (checkpoint, single-group peak, total transport, steps)
...
def _expected(capacity, groups, budgets, weights):
    """Rank the workload frontier by the documented weighted normalised score."""
...
class RecommendMerkleTransportWorkloadWeightedTest(unittest.TestCase):
        ...
        for capacity, groups, budgets in _WORKLOADS:
            for weights in _WEIGHT_SETS:
                with self.subTest(
                    ...
                    result = recommend_merkle_transport_workload_weighted(
    ...
    def test_zero_weight_ignores_dimension(self):
        ...
        result = recommend_merkle_transport_workload_weighted(
            16, _GROUPS, _BUDGETS, (0, 0, 0, 1)
        )
        self.assertEqual(steps(result), min(steps(workload) for workload in frontier))

    def test_checkpoint_only_weight_minimises_checkpoint(self):
        frontier = merkle_transport_workload_frontier(16, _GROUPS, _BUDGETS)
        result = recommend_merkle_transport_workload_weighted(
            16, _GROUPS, _BUDGETS, (1, 0, 0, 0)
        )
        self.assertEqual(
            result.config.checkpoint_bytes,
            min(workload.config.checkpoint_bytes for workload in frontier),
        )
                ...
                with self.subTest(
                    ...
                    def score(workload):
                        ...
                        for value, weight, (low, high) in zip(
                            _metrics(workload), weights, spans
                        ):
                            if high > low:
    ...
    def test_budgets_are_honoured(self):
        ...
        result = recommend_merkle_transport_workload_weighted(
            capacity, groups, budgets, (1, 2, 3, 4)
        )
    ...
    def test_all_four_parameters_are_required_without_defaults(self):
        sig = inspect.signature(recommend_merkle_transport_workload_weighted)
        self.assertEqual(
            list(sig.parameters),
            ["capacity", "groups", "budgets", "weights"],
        )
    ...
    def test_pure_no_state_change(self):
        ...
        for weights in _WEIGHT_SETS:
            first = recommend_merkle_transport_workload_weighted(
                16,
                _GROUPS,
                (None, None, None, 9000),
                weights,
            )
            second = recommend_merkle_transport_workload_weighted(
                16,
                _GROUPS,
                (None, None, None, 9000),
                weights,
