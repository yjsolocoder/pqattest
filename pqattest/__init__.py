...
advance_merkle_auth_state is the stateless restore-void-wrap conversion: it
authenticates a Merkle v2 envelope and restores its v1 payload, advances
the restored signer's next_index per advance_to semantics, wraps the
advanced checkpoint at generation g+1 and, only once every output exists,
performs one paired claim over the (g, g+1) transition — an equal target is
legal and still yields the next-generation envelope.
...
def advance_merkle_auth_state(
    ...
    """Restore a Merkle v2 envelope, void leaves, and wrap the advanced state.

    The stateless counterpart of
    :meth:`MerkleSigner.advance_to_with_auth_state`: combines authenticated
    v2 restore, one :meth:`MerkleSigner.advance_to`-style leaf voiding and
    ...
    :meth:`MerkleSigner.from_auth_state`; the restored signer's
    ``next_index`` is then advanced to ``next_index`` under exactly the
    :meth:`MerkleSigner.advance_to` rules. ``key``, ``min_generation`` and
    ``claim`` are keyword-only; only ``min_generation`` has a default
    (``None``, no floor). Returns ``((before, after), envelope, generation)``:
    the inner tuple is the next-leaf index before and after the jump (the
    original index and the target, exactly what
    :meth:`MerkleSigner.advance_to` returns for the same target), the
    :func:`auth_state_wrap` v2 envelope (``bytes``) over the advanced v1
    :meth:`MerkleSigner.checkpoint` bytes with ``scheme="merkle"``, the
    original ``key`` and generation ``g + 1`` — byte-for-byte identical to
    calling :func:`auth_state_wrap` on the checkpoint a signer with the
    advanced ``next_index`` returns from :meth:`checkpoint` — and the new
    generation ``g + 1``. An equal target is legal: the state does not move,
    but a next-generation envelope authenticating that state is still
    produced.

    ``data`` must be ``bytes`` or ``bytearray`` holding a v2 envelope;
    ``next_index`` must be a non-boolean integer in the closed interval
    ``[current value, leaf count]`` of the restored signer; ``key`` must be a
    non-empty ``bytes``/``bytearray`` shared secret; ``min_generation`` must
    be ``None`` or a non-boolean integer in ``0 .. 2**64 - 1``; ``claim``
    must be callable. A wrong type (including a boolean target or floor, or a
    non-callable claim) raises ``TypeError``. An empty key, a bad envelope, a
    target below the restored index or above the leaf count, or a generation
    ``g`` at the uint64 ceiling (it must be strictly below ``2**64 - 1`` so
    the advanced generation fits) raises ``ValueError``. The v2 HMAC tag is
    verified first with :func:`hmac.compare_digest`, the envelope is then
    fixed to ``"merkle"`` and the generation floor applied, only afterwards
    is the untouched v1 payload restored via
    :meth:`MerkleSigner.from_checkpoint` and advanced per
    :meth:`MerkleSigner.advance_to` semantics, the candidate advanced
    checkpoint/envelope are built, and only once every output exists is
    ...
    invokes the callback or returns a partial result. No randomness is drawn,
    no input is mutated, and the same inputs always produce byte-identical
    outputs. The envelope is plaintext and authenticated only; it provides
