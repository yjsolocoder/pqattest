...
- `recommend_scheme(capacity, budgets, prefer="size")` — **跨 Lamport、W-OTS、Merkle 三方案的统一推荐入口**，在预算内选出一个方案/参数组合并返回其 `Params`。纯函数：不取随机数、不生成密钥、不改状态，同一输入重复调用结果逐项相同；`capacity` 与 `budgets` 无默认值，`prefer` 缺省为 `"size"`。候选覆盖 `profile` 接受的全部方案与参数组合：`capacity == 1` 时含 Lamport 与两档 `w`（4、8）的 W-OTS（一次性方案只签一条），条数大于一时只剩 Merkle；Merkle 候选为 `w=4/8` × `height=1..8` 中 `capacity`（`Params.capacity`）覆盖请求条数者，请求条数本身不得超过最大 Merkle 容量 256。`budgets` 必须为二元组，按顺序分别限制单签序列化尺寸（`Params.sig_bytes`，Merkle 含认证路径，口径与 `profile` 完全一致）与验签链步上界（`Params.steps`）的含边界上限，各项为 `None`（不限）或正的非布尔整数，且至少一项非空；超出任一设限预算的候选一律排除。`prefer="size"` 先最小化单签尺寸再看链步，`prefer="speed"` 先最小化链步再看单签尺寸；两种偏好决胜尾序相同，依次为容量余量最小（`Params.capacity - capacity`）、方案名字典序、`w` 升序、`height` 升序，没有该参数（`None`）者排在最前；返回值是 `Params`，可直接读出方案、参数、容量与两项成本。校验顺序固定为容量、预算、偏好：多处同时非法时按此次序先报；`capacity` 为布尔、非整数或越界抛 `ValueError`；`budgets` 非元组抛 `TypeError`，长度不符、成员为布尔或非正或非整数、两项全空抛 `ValueError`；`prefer` 取值之外或无任何可行候选同样抛 `ValueError`
...
# 跨方案统一入口：预算顺序为 (单签序列化尺寸, 验签链步上界)，不限的项传 None；
# 条数大于 1 时只剩 Merkle 候选，size 先压单签尺寸、speed 先压链步
recommend_scheme(1, (None, 9000))
# Params(scheme='wots', w=8, ..., sig_bytes=1088, steps=8670)：单签最短
recommend_scheme(1, (None, 9000), prefer="speed")
# Params(scheme='lamport', ..., sig_bytes=8192, steps=0)：链步最少
recommend_scheme(16, (1300, None))
# Params(scheme='merkle', w=8, height=4, ..., sig_bytes=1216, steps=8670)
