...
class AdvanceAndSignMerkleAuthStateTest(unittest.TestCase):
    ...
    def test_returns_inner_tuple_signature_envelope_and_generation(self):
    ...
    def test_signature_matches_advance_then_sign_from_same_state(self):
    ...
    def test_signs_target_leaf_of_restored_state(self):
        ...
        (before, target), signature, _, generation = (
            advance_and_sign_merkle_auth_state(
                envelope, 6, "claim", key=KEY, claim=accept
            )
        )
        self.assertEqual((before, target), (1, 6))
        self.assertEqual(signature, reference.sign("claim"))
        self.assertEqual(signature.index, 6)
        self.assertEqual(generation, 6)
        self.assertTrue(merkle_verify("claim", signature, signer.public_key))
    ...
    def test_returned_envelope_unwraps_to_post_signature_state(self):
        (before, target), signature, envelope, generation = (
            advance_and_sign_merkle_auth_state(
                self.envelope, 2, b"one", key=KEY, claim=accept
            )
        )
        ...
        # The restored signer resumes on the leaf after the signed target.
        follow_up = restored.sign(b"two")
        self.assertEqual(follow_up.index, 3)
        self.assertTrue(merkle_verify(b"two", follow_up, self.signer.public_key))
    ...
    def test_equal_target_is_legal_and_signs_current_leaf(self):
        ...
        reference = make_signer()
        reference.advance_to(3)
        (before, target), signature, out_envelope, generation = (
            advance_and_sign_merkle_auth_state(
    ...
    def test_last_leaf_is_a_legal_target(self):
        signer = make_signer(height=2)  # 4 leaves
        envelope = wrap_merkle(signer, generation=0)
        (before, target), signature, out_envelope, generation = (
            advance_and_sign_merkle_auth_state(
                envelope, 3, b"last", key=KEY, claim=accept
            )
        )
        self.assertEqual((before, target), (0, 3))
        self.assertEqual(signature.index, 3)
    ...
    def test_accepts_bytearray_inputs(self):
        result = advance_and_sign_merkle_auth_state(
            bytearray(self.envelope), 1, bytearray(b"m"),
    ...
    def test_stateless_chaining_through_envelopes(self):
        ...
        for target, message in zip(targets, messages):
            _, signature, envelope, generation = (
                advance_and_sign_merkle_auth_state(
        ...
        self.assertEqual(tuple(sig.index for sig in signatures), (1, 2, 3))
    ...
    def test_backwards_target_raises_value_error_without_claim(self):
        ...
        (before, target), _, _, generation = advance_and_sign_merkle_auth_state(
    ...
    def test_exhausted_restored_state_raises_key_exhausted_without_claim(self):
        signer = make_signer(height=2)
        signer.advance_to(4)
        ...
        with self.assertRaises(KeyExhaustedError):
            advance_and_sign_merkle_auth_state(
                envelope, 4, b"m", key=KEY, claim=claim
            )
        self.assertEqual(claim.calls, [])
...
class AdvanceAndSignMerkleAuthStateTypesTest(unittest.TestCase):
    ...
    def test_claim_is_required(self):
        with self.assertRaises(TypeError):
            advance_and_sign_merkle_auth_state(self.envelope, 3, b"m", key=KEY)
    ...
    def test_types_checked_before_envelope(self):
        with self.assertRaises(TypeError):
            advance_and_sign_merkle_auth_state(b"", 3, b"m", key=42, claim=accept)
        with self.assertRaises(TypeError):
            advance_and_sign_merkle_auth_state(
        ...
        with self.assertRaises(TypeError):
            advance_and_sign_merkle_auth_state(b"", True, b"m", key=KEY, claim=accept)
        with self.assertRaises(TypeError):
            advance_and_sign_merkle_auth_state(b"", 3, 42, key=KEY, claim=accept)
