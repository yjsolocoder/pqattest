...
:func:`recommend_merkle_deployment_weighted` ranks that same frontier by a
caller-supplied four-tuple of non-negative weights over its four
min-max-normalised costs in the order checkpoint bytes, single-signature
wire length, standalone-proof wire length and verifier steps (the weighted
sum divided by the weight total), with exact :class:`fractions.Fraction`
arithmetic, and returns one :class:`MerkleStorageProfile`. :func:`merkle_verify_profile` compares the
...
def recommend_merkle_deployment_weighted(
    capacity: Any, budgets: Any, weights: Any
) -> MerkleStorageProfile:
    """Pick one ordinary Merkle deployment by weighted normalised costs.

    Computes :func:`merkle_deployment_frontier` exactly once and ranks the
    returned feasible, non-dominated deployments by a caller-supplied linear
    ...
    single-signature wire bytes, standalone-proof wire bytes and
    ...
    weighted score is the sum of the four normalised costs times their
    ...
    weight raises ``ValueError``. The returned profile is one of the
    profiles :func:`merkle_deployment_frontier` returns for the same
    arguments. The function is pure: it draws no randomness, generates no
    keys and changes no state.
    """
