...
decision-cost breakdown behind that same single-weight joint-deployment
ranking as a tuple of frozen :class:`MerkleTransportDeploymentScore`
rows — one per frontier member, in frontier order, each carrying the
candidate's deployment profile, its five normalised costs, its final
score and a selected flag — with the same exact
...
class MerkleTransportDeploymentScore:
    """Frozen decision-cost breakdown for one joint deployment.
