...
_WORKLOADS = (
    ...
    (2, ((0,), (1,)), (None, 4000, None, None)),
    (32, ((1, 7), (16, 20)), (None, None, None, 9000)),
...
class ExplainMerkleTransportWorkloadWeightedTest(unittest.TestCase):
    ...
    def test_row_order_matches_frontier_member_order(self):
        frontier = merkle_transport_workload_frontier(4, _GROUPS, _BUDGETS)
    ...
    def test_invalid_weights_members_raise(self):
        bad_bases = (
            (),
            (1, 1, 1),
            (1, 1, 1, 1, 1),
        ...
        # a boolean or negative member is a ValueError and a non-integer
        # member is a TypeError at every position, not just the first one
    ...
    def test_frontier_arguments_validated_like_frontier(self):
        ...
        for bad_groups in ([(0, 1)], {(0, 1)}, "groups", None, 7):
            with self.subTest(bad_groups=bad_groups):
                with self.assertRaises(TypeError):
                    explain_merkle_transport_workload_weighted(
                        4, bad_groups, _BUDGETS, (1, 1, 1, 1)
                    )
        with self.assertRaises(TypeError):
            explain_merkle_transport_workload_weighted(
                4, ([0, 1],), _BUDGETS, (1, 1, 1, 1)
            )
        with self.assertRaises(ValueError):
            explain_merkle_transport_workload_weighted(
                4, (), _BUDGETS, (1, 1, 1, 1)
        ...
        with self.assertRaises(ValueError):
            explain_merkle_transport_workload_weighted(
                4, ((2, 1),), _BUDGETS, (1, 1, 1, 1)
            )
        for bad_budgets in ([None] * 4, "budget", None, 7, {1, 2}):
            with self.subTest(bad_budgets=bad_budgets):
                with self.assertRaises(TypeError):
                    explain_merkle_transport_workload_weighted(
                        4, _GROUPS, bad_budgets, (1, 1, 1, 1)
                    )
        with self.assertRaises(ValueError):
            explain_merkle_transport_workload_weighted(
                4, _GROUPS, (None,) * 4, (1, 1, 1, 1)
            )
        with self.assertRaises(ValueError):
            explain_merkle_transport_workload_weighted(
                4, _GROUPS, (None,) * 3, (1, 1, 1, 1)
            )
