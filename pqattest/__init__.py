...
(g, g+1) claim that runs only after every output exists.
advance_and_sign_merkle_auth_state_batch is the batch counterpart: it
restores the same way, advances the next-leaf index to a caller-chosen
target that must remain signable, signs a non-empty tuple of messages with
consecutive leaves starting at the target in a single sign_batch run, and
returns the (before, target) index pair, the signature tuple, the envelope
and generation g+1 under the same single (g, g+1) claim.
...
def advance_and_sign_merkle_auth_state_batch(
    ...
    """Restore a v2 envelope, void leaves to ``next_index``, sign a batch there.

    The batch counterpart of :func:`advance_and_sign_merkle_auth_state`:
    combines authenticated v2 restore, an :meth:`MerkleSigner.advance_to`
    -semantics jump to ``next_index``, one contiguous
    :meth:`MerkleSigner.sign_batch` run starting at that target leaf and
    wrapping of the advanced v1 checkpoint in a single call without mutating
    any object, keeping any library state or introducing a new wire format.
    The envelope in ``data`` is authenticated and restored exactly like
    :meth:`MerkleSigner.from_auth_state`; the restored signer then advances
    its next-leaf index to ``next_index`` exactly like
    :meth:`MerkleSigner.advance_to`, and signs every message in
    ``messages`` on the consecutive leaves starting at the target, exactly
    like ``advance_to(target)`` followed by ``sign_batch(messages)`` from
    the advanced state. ``key``, ``min_generation`` and ``claim`` are
    keyword-only; only ``min_generation`` has a default (``None``, no
    floor). Returns ``((before, target), signatures, envelope,
    generation)``: the inner tuple is the next-leaf index before the jump
    and the target, exactly what :meth:`MerkleSigner.advance_to` returns
    for the same target (an equal target is legal and returns the same
    value twice); ``signatures`` is the tuple of one :class:`MerkleSignature`
    per message, in order, with strictly increasing indices starting at
    ``target`` — value-for-value identical to
    :meth:`MerkleSigner.sign_batch` after :meth:`MerkleSigner.advance_to`
    from the same state; ``envelope`` is the :func:`auth_state_wrap` v2
    envelope (``bytes``) over the v1 :meth:`MerkleSigner.checkpoint` bytes
    with ``next_index == target + len(messages)``, ``scheme="merkle"``,
    the original ``key`` and generation ``g + 1`` — byte-for-byte
    identical to calling :func:`auth_state_wrap` on the checkpoint a
    signer advanced to ``target`` and then batch-signed returns from
    :meth:`checkpoint` — and ``generation`` is the new generation
    ``g + 1``.

    ``data`` must be ``bytes`` or ``bytearray`` holding a v2 envelope;
    ``next_index`` must be a non-boolean integer in the closed interval
    ``[current next_index, leaf count - 1]`` — unlike
    :func:`advance_merkle_auth_state`, the leaf count itself is not a
    legal target because the target leaf must still be signable;
    ``messages`` must be a **non-empty** ``tuple`` whose members each
    follow the usual ``bytes``/``bytearray``/``str`` (UTF-8) message
    rules; ``key`` must be a non-empty ``bytes``/``bytearray`` shared
    secret; ``min_generation`` must be ``None`` or a non-boolean integer
    in ``0 .. 2**64 - 1``; ``claim`` must be callable. A wrong type
    (including a non-tuple ``messages``, a bad message member, a boolean
    target or floor, or a non-callable claim) raises ``TypeError``. The
    v2 HMAC tag is verified first with :func:`hmac.compare_digest`, the
    envelope is fixed to ``"merkle"`` and the generation floor applied,
    the v1 checkpoint is restored untouched, and the target is
    range-checked against the restored state — a target below the
    restored current index or above the last leaf raises ``ValueError``,
    as does an empty batch, an empty ``key`` or an input generation ``g``
    at the uint64 ceiling (it must be strictly below ``2**64 - 1`` so
    ``g + 1`` fits). A restored signer with no leaf left, or without
    enough leaves from the target for the whole batch, raises
    :class:`KeyExhaustedError`. Only once every output exists is
    ``claim`` called exactly once with the paired token
    ``(("merkle", g), ("merkle", g + 1))``; the call succeeds only when
    that return value ``is True`` — otherwise it raises ``ValueError`` —
    and any exception ``claim`` raises propagates untouched. No earlier
    failure invokes the callback, advances any state or returns a
    partial result. No object is mutated, the same inputs always produce
    ...
    signer = MerkleSigner.from_checkpoint(checkpoint)
    # Build every output before the claim: an exhausted state, an
    # out-of-range target, an under-capacity batch or any failure here
    # must leave no partial result and must not call claim.
    with signer._lock:
        before = signer._next_index
        leaf_count = len(signer._private_keys)
        if before >= leaf_count:
