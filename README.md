...
- `advance_and_multiproof_merkle_auth_state(data, next_index, messages, *, key, min_generation=None, claim) -> ((before, target), proof, envelope, generation)` — **无隐藏状态**的「认证恢复 + 跳叶作废 + 自目标叶起连续批量签名 + 去重多证明 + 下一代封装」转换，是 `advance_and_sign_merkle_auth_state_batch` 的多证明对应物，旧接口与线格式一律不变：`data` 为 `"merkle"` 的 v2 封装（`bytes`/`bytearray`），先按既有 v2 规则用 `hmac.compare_digest` 验 HMAC、固定方案、应用代次下限并恢复原样 v1 载荷，再按 `MerkleSigner.advance_to` 的既有语义把恢复签名器的下一可用叶索引推进到 `next_index`，随后按 `MerkleSigner.sign_batch` 的既有规则从**目标叶**起对 `messages` **连续**签名（一次临界区、叶索引自 `target` 起严格递增），并对恢复公钥与这批签名调用 `multiproof_encode` 生成**一份**共享兄弟节点去重的多证明，最后把 `next_index == target + len(messages)` 的 v1 检查点以 `scheme="merkle"`、原 `key` 与 **g+1** 代次封装。`messages` 必须是**非空元组**，成员沿用 `bytes`/`bytearray`/`str`（UTF-8）规则；`next_index` 须为闭区间 `[恢复状态的当前 next_index, 叶总数 - 1]` 内的非布尔整数（叶总数本身**不是**合法目标，目标叶必须仍可签）；其余参数、异常与 `claim` 规则完全沿用 `advance_and_sign_merkle_auth_state_batch`（错型抛 `TypeError`；倒退或越过最后一片叶、空批、入参 `g` 触顶、证明超出 v1 格式表达力抛 `ValueError`；目标叶可签但整批超出剩余叶数抛 `KeyExhaustedError`）。返回 `((before, target), proof, envelope, generation)`：内层二元组与同目标下 `advance_to` 的返回相同（等值目标合法，返回 `(x, x)`）；`proof` 为 `bytes`，与「同状态先 `advance_to(target)` 再 `sign_batch(messages)` 后对该签名器公钥与签名元组调用 `multiproof_encode`」**逐字节相同**，且 `multiproof_verify(messages, proof)` 返回 `True`；`envelope` 与对批签后检查点直接调用 `auth_state_wrap` **逐字节相同**；`generation` 为 `g+1`，状态只推进一代（与批长无关）。不修改任何对象、不保留库内状态、不取随机数、不新增线格式，同一输入重复调用逐字节相同。**全部输出生成后**才以唯一入参 `(("merkle", g), ("merkle", g+1))` 恰好调用一次 `claim`，仅返回值 `is True` 时成功（否则抛 `ValueError`），回调异常原样透传；任何先前失败（含错型、空批、空 key、认证失败、代次低于下限、代次触顶、目标越界、容量不足、证明不可编码、检查点非法、认领拒绝）都**不调用** `claim`、不推进且无部分返回
...
同样的跳叶 + 整批签名，若只需一份去重的传输证明而不要签名元组，用 `advance_and_multiproof_merkle_auth_state`：返回的 `proof` 是 `multiproof_encode` 的 v1 字节（共享兄弟节点只携带一次），可直接交给 `multiproof_verify`，其余恢复、跳叶、容量检查、代次与单次 `(g, g+1)` 认领语义与批量版完全一致：
...
messages = (b"claim 5", b"claim 6", b"claim 7")
(before, target), proof, next_blob, generation = (
    advance_and_multiproof_merkle_auth_state(
        blob, 5, messages, key=key,
        min_generation=state["high_water"], claim=claim_transition,
    )
)
...
assert (before, target) == (0, 5)     # 叶子 0..4 已作废
assert generation == 8
assert multiproof_verify(messages, proof)
assert not multiproof_verify((b"claim 5", b"claim 6", b"other"), proof)
```
