...
import pqattest
from pqattest import KeyExhaustedError, OneTimeSigner, keygen, verify
...
def make_signer(bits=16, start=0):
...
class CheckpointFormatTest(unittest.TestCase):
    def test_layout_and_length(self):
        for bits in (1, 7, 16, 256):
            with self.subTest(bits=bits):
                signer = make_signer(bits=bits)
                blob = signer.checkpoint()
                key_bytes = signer._private_key.to_bytes()
                header = 8 + 1 + 1 + 4
                self.assertEqual(len(blob), header + len(key_bytes) + 32)
                ...
                self.assertEqual(
                    int.from_bytes(blob[10:14], "big"), len(key_bytes)
                )
                self.assertEqual(blob[header : header + len(key_bytes)], key_bytes)
    ...
    def test_same_state_encodes_identically(self):
        first = make_signer(bits=16, start=0)
...
class CheckpointRoundTripTest(unittest.TestCase):
    ...
    def test_private_key_secrets_preserved_in_order(self):
        ...
        restored = OneTimeSigner.from_checkpoint(signer.checkpoint())
        self.assertEqual(
            restored._private_key.secrets, signer._private_key.secrets
        )
...
class CheckpointValidationTest(unittest.TestCase):
    ...
    def test_truncated_and_empty_rejected(self):
        ...
        self.assert_rejected(self.blob[:13])
    ...
    def test_bad_key_length_rejected(self):
        key_length = len(self.signer._private_key.to_bytes())
        for count in (0, key_length - 1, key_length + 1, 2 * key_length):
            with self.subTest(count=count):
                bad = bytearray(self.blob)
                bad[10:14] = count.to_bytes(4, "big")
                self.assert_rejected(bytes(bad))

    def test_nested_private_key_encoding_invalid_rejected(self):
        # Corrupt the nested encoding's magic and recompute the outer
        # checksum: the nested parse must still fail.
        bad = bytearray(self.blob)
        bad[14] ^= 0x01
